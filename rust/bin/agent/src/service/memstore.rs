...
mod tests {
    ...
    async fn test_get_object_updates_kvs_timestamp() {
        ...
        kv.set("uuid1".to_string(), 42, 100).await.unwrap();
        vq.push_insert("uuid1", vec![1.0, 2.0], Some(200)).await.unwrap();
    ...
    async fn test_get_object_updates_kvs_timestamp_from_insert_ts() {
        ...
        vq.push_insert("uuid1", vec![1.0, 2.0, 3.0], Some(200)).await.unwrap();
        vq.push_delete("uuid1", Some(300)).await.unwrap();
        ...
        assert_eq!(oid, 42);
        assert_eq!(kts, 200, "kvs timestamp should be updated to vqueue insert timestamp");
    }
    ...
    async fn test_get_object_vector_fn_returns_error() {
        ...
        let get_fn = |_oid: u32| async move {
            Err(MemstoreError::ObjectNotFound("vector not found".to_string()))
        };
    ...
    async fn test_update_timestamp_empty_uuid() {
        ...
        let result = update_timestamp::<_, NoopFn, NoopFuture>(&kv, &vq, "", 200, false, None).await;
    ...
    async fn test_update_timestamp_zero_timestamp_without_force() {
        ...
        let result = update_timestamp::<_, NoopFn, NoopFuture>(&kv, &vq, "uuid1", 0, false, None).await;
    ...
    async fn test_update_timestamp_both_vqueue_and_kvs() {
        ...
        kv.set("uuid1".to_string(), 42, 100).await.unwrap();
        vq.push_insert("uuid1", vec![1.0, 2.0], Some(150)).await.unwrap();
    ...
    async fn test_update_timestamp_force_older_than_both() {
        ...
        kv.set("uuid1".to_string(), 42, 200).await.unwrap();
        vq.push_insert("uuid1", vec![1.0, 2.0], Some(300)).await.unwrap();
    ...
    async fn test_get_object_prefers_vqueue_over_kvs() {
        ...
        kv.set("uuid1".to_string(), 42, 100).await.unwrap();
        // New data in vqueue
        vq.push_insert("uuid1", vec![999.0], Some(200)).await.unwrap();
    ...
    async fn test_concurrent_operations() {
        ...
        let handles: Vec<_> = (0..10).map(|i| {
            ...
            tokio::spawn(async move {
                ...
                vq.push_insert(&uuid, vec![i as f32], Some(100 + i as i64)).await.unwrap();
                kv.set(uuid.clone(), i as u32, (100 + i) as u128).await.unwrap();
            })
        }).collect();
    ...
    async fn test_special_characters_in_uuid() {
        ...
        for (i, uuid) in special_uuids.iter().enumerate() {
            vq.push_insert(*uuid, vec![i as f32], Some(100 + i as i64)).await.unwrap();
            kv.set(uuid.to_string(), i as u32, (100 + i) as u128).await.unwrap();
        }
    ...
    async fn test_large_vector_handling() {
        ...
        vq.push_insert("uuid1", large_vec.clone(), Some(100)).await.unwrap();
    ...
    async fn test_max_timestamp_handling() {
        ...
        let max_ts = i64::MAX;
        vq.push_insert("uuid1", vec![1.0], Some(max_ts)).await.unwrap();
