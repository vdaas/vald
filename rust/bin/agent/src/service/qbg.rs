                ...
                for (obj_id, distance) in results {
                    match self.kvs.get_inverse(&obj_id).await {
                        Ok((uuid, _)) => {
                            ...
                            if !is_deleted {
                                distance_results.push(Distance { id: uuid, distance });
                            } else {
...
mod tests {
    ...
    async fn test_get_object_from_vqueue() {
        ...
        assert!(
            res.is_ok(),
            "Insert with time should succeed: {:?}",
            res.err()
        );
    ...
    async fn test_update_timestamp_with_remove_and_reinsert() {
        ...
        // Insert first vector
        let res = test_svc.service.insert(uuid.clone(), vector1).await;
        assert!(res.is_ok(), "Initial insert should succeed");
    ...
    async fn test_boundary_zero_timestamp() {
        ...
        let result = test_svc
            ...
            .await;
        assert!(result.is_ok(), "Insert with zero timestamp should succeed");
    }
    ...
    async fn test_boundary_remove_nonexistent_uuid() {
        ...
        assert!(result.is_err(), "Remove non-existent UUID should fail");
    }
    ...
    async fn test_boundary_insert_and_get_many_times() {
        ...
        // Insert once
        let res = test_svc.service.insert(uuid.clone(), vector.clone()).await;
        assert!(res.is_ok(), "Initial insert should succeed");
