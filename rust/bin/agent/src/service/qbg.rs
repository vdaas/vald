...
impl QBGService {
    fn new(settings: Config) -> Self {
        ...
        let vqueue = vqueue::Builder::new(path).build().await.unwrap();
        QBGService {
            path,
            index,
            property,
    ...
    fn _insert(&mut self, uuid: String, vector: Vec<f32>, t: i64, validation: bool) -> Result<(), Error> {
        if uuid.len() == 0 {
            return Err(Error::UUIDNotFound {
                uuid: "0".to_string(),
            });
        ...
        if validation {
            let (_, ok) = self.exists(uuid.clone());
            if ok {
                return Err(Error::UUIDAlreadyExists { uuid });
            }
        }
        self.insert_with_time(uuid, vector, t)?;
        Ok(())
    }

    fn _insert_multiple(&mut self, vectors: HashMap<String, Vec<f32>>, t: i64, validation: bool) -> Result<(), Error> {
        for (uuid, vec) in vectors {
            if validation {
                self.ready_for_update(uuid.clone(), vec.clone(), t)?;
            }
            self.insert_with_time(uuid, vec, t)?;
        }
        Ok(())
    }
}
...
impl ANN for QBGService {
    fn exists(&self, _uuid: String) -> (usize, bool) {
        // convert uuid to id
        let id = 1;
    ...
    fn insert(&mut self, _uuid: String, vector: Vec<f32>) -> Result<(), Error> {
    ...
    fn insert_multiple(&mut self, vectors: HashMap<String, Vec<f32>>) -> Result<(), Error> {
        ...
        for (uuid, vec) in vectors {
            let result = self.insert(uuid, vec);
            match result {
                Ok(()) => continue,
                Err(err) => match err {
