...
pub mod sidecar_server {
    ...
    #[async_trait]
    pub trait Sidecar: std::marker::Send + std::marker::Sync + 'static {}
    #[derive(Debug)]
    pub struct SidecarServer<T> {
        inner: Arc<T>,
    ...
    impl<T> Clone for SidecarServer<T> {
        fn clone(&self) -> Self {
    ...
    pub const SERVICE_NAME: &str = "sidecar.v1.Sidecar";
    impl<T> tonic::server::NamedService for SidecarServer<T> {
