...
pub mod discoverer_server {
    ...
    impl<T> Clone for DiscovererServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "discoverer.v1.Discoverer";
    impl<T> tonic::server::NamedService for DiscovererServer<T> {
