...
pub mod meta_server {
    ...
    impl<T: Meta> Clone for MetaServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Meta> tonic::server::NamedService for MetaServer<T> {
        const NAME: &'static str = "meta.v1.Meta";
    }
}
