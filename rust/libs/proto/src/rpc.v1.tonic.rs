...
pub mod stats_server {
    ...
    #[derive(Debug)]
    pub struct StatsServer<T: Stats> {
        inner: Arc<T>,
    ...
    impl<T: Stats> Clone for StatsServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Stats> tonic::server::NamedService for StatsServer<T> {
        const NAME: &'static str = "rpc.v1.Stats";
    }
}
