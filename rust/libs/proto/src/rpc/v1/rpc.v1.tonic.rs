...
pub mod stats_client {
    ...
    impl StatsClient<tonic::transport::Channel> {
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        pub fn with_interceptor<F>(
            inner: T,
            interceptor: F,
        ) -> StatsClient<InterceptedService<T, F>>
        where
...
pub mod stats_server {
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for StatsServer<T>
    where
        T: Stats,
    ...
    impl<T> Clone for StatsServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "rpc.v1.Stats";
    impl<T> tonic::server::NamedService for StatsServer<T> {
        const NAME: &'static str = SERVICE_NAME;
    }
}
/// Generated client implementations.
pub mod stats_detail_client {
    #![allow(
        unused_variables,
        dead_code,
        missing_docs,
    ...
    impl StatsDetailClient<tonic::transport::Channel> {
        /// Attempt to create a new client by connecting to a given endpoint.
        pub async fn connect<D>(dst: D) -> Result<Self, tonic::transport::Error>
        where
            D: TryInto<tonic::transport::Endpoint>,
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        pub fn with_interceptor<F>(
            inner: T,
            interceptor: F,
        ) -> StatsDetailClient<InterceptedService<T, F>>
        where
            F: tonic::service::Interceptor,
            T::ResponseBody: Default,
            T: tonic::codegen::Service<
...
pub mod stats_detail_server {
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for StatsDetailServer<T>
    where
        T: StatsDetail,
        B: Body + std::marker::Send + 'static,
        B::Error: Into<StdError> + std::marker::Send + 'static,
    ...
    impl<T> Clone for StatsDetailServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    /// Generated gRPC service name
    pub const SERVICE_NAME: &str = "rpc.v1.StatsDetail";
    impl<T> tonic::server::NamedService for StatsDetailServer<T> {
        const NAME: &'static str = SERVICE_NAME;
    }
}
