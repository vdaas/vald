...
pub mod property {
    ...
    impl Property {
        ...
        pub fn set_number_of_second_objects(&mut self, number_of_second_objects: u32) {
            self.inner
                .pin_mut()
...
pub mod index {
    ...
    impl Index {
        pub fn new(path: &String, p: &mut property::Property) -> Result<Self, cxx::Exception> {
            let inner = match ffi::new_index(path, p.get_property()) {
                Ok(inner) => inner,
                Err(e) => return Err(e),
            };
            Ok(Index { inner })
        ...
        pub fn new_prebuilt(path: &String, p: bool) -> Result<Self, cxx::Exception> {
            let inner = match ffi::new_prebuilt_index(path, p) {
                Ok(inner) => inner,
                Err(e) => return Err(e),
            };
            Ok(Index { inner })
        ...
        pub fn get_object(&mut self, id: usize) -> Result<&[f32], cxx::Exception> {
            let dim = match self.inner.pin_mut().get_dimension() {
                Ok(dim) => dim,
                Err(e) => return Err(e),
            };
...
mod tests {
    ...
    fn test_ffi_qbg() -> Result<()> {
        ...
        for i in 0..100 {
            ...
            let id = match index.pin_mut().append(vec.as_slice()) {
                Ok(id) => id,
                Err(e) => panic!("{}", e),
            };
            assert_eq!((i + 1) as i32, id)
        }
        match index.pin_mut().save_index() {
            Ok(_) => {}
            Err(e) => panic!("{}", e),
        };
        ...
        match index.pin_mut().build_index(&path, p.pin_mut()) {
            Ok(_) => {}
            Err(e) => panic!("{}", e),
        };
        match index.pin_mut().open_index(&path, true) {
            Ok(_) => {}
            Err(e) => panic!("{}", e),
        };
        ...
        for i in 0..100 {
            let vec: Vec<f32> = (0..DIMENSION).into_iter().map(|x| (x + i) as f32).collect();
            let id = match index.pin_mut().insert(vec.as_slice()) {
                Ok(id) => id,
                Err(e) => panic!("{}", e),
            };
        ...
        // Get Object
        let vec = match index.pin_mut().get_object(1) {
            Ok(vec) => vec,
            Err(e) => panic!("{}", e),
        };
        println!("vec:\n\t{:?}", vec);
        ...
        // Get Dimension
        let dim = match index.pin_mut().get_dimension() {
            Ok(dim) => dim,
            Err(e) => panic!("{}", e),
        };
        println!("dimension:\n\t{:?}", dim);
    ...
    fn test_index() -> Result<()> {
        ...
        let mut index = match Index::new(&path, &mut p) {
            Ok(index) => index,
            Err(e) => panic!("{}", e),
        };
        ...
        for i in 0..100 {
            let vec: Vec<f32> = (0..DIMENSION).into_iter().map(|x| (x + i) as f32).collect();
            let id = match index.append(vec.as_slice()) {
                Ok(id) => id,
                Err(e) => panic!("{}", e),
            };
            assert_eq!((i + 1) as i32, id)
        ...
        // Build
        println!("building the index...");
        match index.build_index(&path, &mut p) {
            Ok(_) => {}
            Err(e) => panic!("{}", e),
        };
        match index.open_index(&path, true) {
            Ok(_) => {}
            Err(e) => panic!("{}", e),
        };
        ...
        for i in 0..100 {
            let vec: Vec<f32> = (0..DIMENSION).into_iter().map(|x| (x + i) as f32).collect();
            let id = match index.insert(vec.as_slice()) {
                Ok(id) => id,
                Err(e) => panic!("{}", e),
            };
            assert_eq!((i + 1 + 100) as i32, id)
        ...
        // Get Object
        let vec = match index.get_object(1) {
            Ok(vec) => vec,
            Err(e) => panic!("{}", e),
        };
        println!("vec:\n\t{:?}", vec);
        ...
        // Get Dimension
        let dim = match index.get_dimension() {
            Ok(dim) => dim,
            Err(e) => panic!("{}", e),
        };
        println!("dimension:\n\t{:?}", dim);
        ...
        println!("search the index for the specified query...");
        let vec: Vec<f32> = (0..DIMENSION).into_iter().map(|i| i as f32).collect();
        let search_results = match index.search(vec.as_slice(), K, RADIUS, EPSILON) {
            Ok(s) => s,
            Err(e) => panic!("{}", e),
        };
