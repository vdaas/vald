...
2. When input is not `...interface{}` but `...[]int`, `...[]string` or etc.
...
However, when we update our test template, the generated unit test functions will not be updated accordingly. To overcome this, we have a placeholder `// NOT IMPLEMENTED BELOW` to indicate whether the test functions are implemented or not. The unit test functions above the placeholder are implemented, and the functions below are not implemented yet.
...
func Test_notImplementedTest(t *testing.T) {
    // this unit test is not implemented yet
}
```
...
When you use the `make gotests/gen` command to generate the unit test function, the unit test function below the placeholder will be re-generated, so before implementing a new unit test function, please move the test function above the placeholder to indicate the test function is implemented and will not be regenerated by the test template.
