...
In Vald, the table-driven test functions are generated by a test template, so everyone can follow the same style of implementing unit tests easily.
We have a placeholder `// NOT IMPLEMENTED BELOW` on each test implementation file to indicate whether the test functions are implemented or not, in case of any test template updates affecting the empty unit test functions.
Before implementing a unit test case, please move the test function above the placeholder to indicate the test function is implemented and will not be regenerated by the test template.
