...
func getDatasetSlices(
	...
	t.Helper()
	if e == nil || e.BaseConfig == nil {
		return nil, nil, nil
	}
	if len(ds.Train) > int(e.Offset)+int(e.Num) {
		train = ds.Train[e.Offset : e.Offset+e.Num]
	} else {
		t.Logf("train data is not enough, offset: %d, num: %d, total: %d", e.Offset, e.Num, len(ds.Train))
	}

	if len(ds.Test) > int(e.Offset)+int(e.Num) {
		test = ds.Test[e.Offset : e.Offset+e.Num]
	} else {
		t.Logf("test data is not enough, offset: %d, num: %d, total: %d", e.Offset, e.Num, len(ds.Test))
	}

	if len(ds.Neighbors) > int(e.Offset)+int(e.Num) {
		neighbors = ds.Neighbors[e.Offset : e.Offset+e.Num]
	} else {
		t.Logf("neighbor data is not enough, offset: %d, num: %d, total: %d", e.Offset, e.Num, len(ds.Neighbors))
	}
