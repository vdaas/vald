...
func (r *runner) processModification(
	t *testing.T, ctx context.Context, train [][]float32, plan *config.Execution,
) {
	t.Helper()
	if plan == nil {
...
func unaryModify[Q, R proto.Message](
	...
	for i, vec := range data {
		...
		eg.Go(func() error {
			...
			if plan.ModificationConfig != nil {
				ts = plan.ModificationConfig.Timestamp
				skip = plan.ModificationConfig.SkipStrictExistCheck
			}
...
func multiModify[Q, R proto.Message](
	...
	for i, vec := range data {
		...
		reqs = append(reqs, addReqs(id, vec, plan.Timestamp, plan.SkipStrictExistCheck))
		...
		if len(reqs) >= int(plan.BulkSize) {
			...
			eg.Go(func() error {
				...
				t.Logf("vectors %v inserted %s", req, res.String())
				return nil
			})
		}
	}
...
func streamModify[S grpc.ClientStream, R proto.Message](
	...
	err = grpc.BidirectionalStreamClient(stream, func() *R {
		...
		req := newReq(id, data[idx], plan.Timestamp, plan.SkipStrictExistCheck)
		return &req
