...
func readDimension(r io.Reader) (dim int32, ok bool, err error) {
	...
	dim = int32(binary.LittleEndian.Uint32(header[:])) //nolint:gosec // bit-for-bit reinterpretation of an on-disk int32 header
	if dim < 0 {
		// A negative dimension is impossible for well-formed x1b data; it means
		// a corrupt or misaligned header (e.g. wrong file format). Reject it as
		// truncated data instead of letting readPayload's
		// make([]byte, int(dim)*elementSize) panic with a negative length.
		return 0, false, ErrTruncatedData
	}
	return dim, true, nil
}
