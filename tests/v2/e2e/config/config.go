...
type PortForward struct {
	...
	PodPort   uint16 `yaml:"pod_port"`  // The port number on the pod.
	LocalPort uint16 `yaml:"pod_port"`  // The local port number; if not set, it defaults to PodPort.
...
func (pf *PortForward) Bind() *PortForward {
	...
	// If PodPort is not set, default it to the localPort constant.
	if pf.PodPort == 0 {
		pf.PodPort = localPort
	}
	// If LocalPort is not set, default it to the same value as PodPort.
	if pf.LocalPort == 0 {
