...
func repeatOverview(builder *dashboard.DashboardBuilder) {
	podStatusPanel := timeseries.NewPanelBuilder().
		Title("Pod Status").
		Span(5).Height(8)
	for legend, kind := range map[string]string{
		"Replicas":                 "deployment",
		"Replicas Available":       "deployment",
		"Replicas Unavailable":     "deployment",
		"Replicas Updated":         "deployment",
		"Current Number Scheduled": "daemonset",
		"Desired Number Scheduled": "daemonset",
		"Number Available":         "daemonset",
		"Number Misscheduled":      "daemonset",
		"Number Ready":             "daemonset",
		"Number Unavailable":       "daemonset",
		"Replicas Current":         "statefulset",
		"Replicas Ready":           "statefulset",
		"Replicas updated":         "statefulset",
	} {
		podStatusPanel.
			WithTarget(prometheusQuery(
				fmt.Sprintf(
					...
					kind,
					strings.ReplaceAll(strings.ToLower(legend), " ", "_"),
					kind,
					kind,
					kind,
				),
			).Format("time_series").LegendFormat(legend)).
			FillOpacity(10)
	}
	builder.
