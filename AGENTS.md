...
1. **Use `internal/` Libraries As you can:** Do not use standard `log`, `errors`, `sync`, or `strings`. Use `github.com/vdaas/vald/internal/**` instead.
...
6. **Pre-Commit Checks:** Code must pass `make license` and `make format` and `make lint` before suggestion.
...
- **FFI & Safety:**
- Use `unsafe` blocks **only** when interacting with C/C++ libraries (NGT/Faiss).
...
- **Security:** Pin actions to specific versions/SHAs (no `@latest`). Set `permissions` to least privilege.
...
**Command List**: You can find a lot of command by `make help`.
