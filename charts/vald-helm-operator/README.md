...
Please upgrade the CRDs first because Helm doesn't have a support to upgrade CRDs.
    ...
    $ kubectl replace -f https://raw.githubusercontent.com/vdaas/vald/v1.7.3/charts/vald-helm-operator/crds/valdrelease.yaml
    $ kubectl replace -f https://raw.githubusercontent.com/vdaas/vald/v1.7.3/charts/vald-helm-operator/crds/valdhelmoperatorrelease.yaml
...
If you're using `valdhelmoperatorrelease` (or `vhor`) resource, please update the `spec.image.tag` field of it.
    ...
    $ kubectl patch vhor vhor-release -p '{"spec":{"image":{"tag":"v1.7.3"}}}'
...
| image.tag | string | `"v1.7.3"` | image tag |
