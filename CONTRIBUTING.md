...
### How to contribute test code

To contribute test code, the steps are almost the same as contribute source code.
However in Vald, we need some extra steps when making changes to test code.

In Vald, the table-driven test functions of the unit test are generated by [gotests](https://github.com/cweill/gotests) using the `make gotests/gen` command.
The same style of implementation is generated for each unit test function, so everyone can follow the same style of implementing unit tests easily.

We implemented our own [test template for gotests](https://github.com/vdaas/vald/tree/main/assets/test/templates) to generate the unit test functions, to provide extra flexibility and functionality for the unit test, such as [goleak](https://github.com/uber-go/goleak) support.

However, when we update our test template, the generated unit test functions will not be updated accordingly.
To overcome this, we implemented a placeholder `// NOT IMPLEMENTED BELOW` to indicate whether the test functions are implemented or not. The unit test functions above the placeholder are implemented, and the functions below are not implemented yet.
...
When the test template is updated, if the unit test function is implemented already, we cannot do anything about it. But for unit test functions that are not implemented yet, we can re-generate them by using the updated template.

Every unit test file should contains the placeholder in the file.
If all of the test functions are implemented, the placeholder will be placed on the bottom of the unit test file.
...
If only part of the test function is implemented, the placeholder will be placed between the implemented and unimplemented function.
...
When you use the `make gotests/gen` command to generate the unit test function, the unit test function below the placeholder will be re-generated.

Please make sure that before implementing a new unit test function, move the test function above the placeholder to indicate the test function is implemented and will not be regenerated by the test template.
