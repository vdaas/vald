...
Vald applies the table-driven test functions of the unit test generated by [gotests](https://github.com/cweill/gotests) using the `make gotests/gen` command.
...
Vald uses its own [test template for gotests](https://github.com/vdaas/vald/tree/main/assets/test/templates) to generate the unit test functions, to provide extra flexibility and functionality for the unit test, such as [goleak](https://github.com/uber-go/goleak) support.

However, when the test template is updated, the generated unit test functions will not be updated accordingly.
To overcome this, each test file has a placeholder `// NOT IMPLEMENTED BELOW` to indicate whether the test functions are implemented.
The unit test functions above the placeholder are implemented, and the functions below still need to be implemented.
If all the test functions are implemented, the placeholder will be placed at the bottom of the unit test file.
...
The implemented unit test is not updated even when the test template is updated.
On the other hand, we have to re-generate unimplemented tests by using the latest test template.
