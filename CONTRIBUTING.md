...
This implementation structure is generated by the `make gotests/gen` command for all unit test files, by appling the custom unit test template using [gotests](https://github.com/cweill/gotests).

When the test template is updated, the generated unit test functions will not be updated accordingly, so we need to have this unit test structure to help us to regenerate the implementation with the latest template.
For the implemented unit test, we cannot deal with it with the `make gotests/gen` command.
