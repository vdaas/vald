...
func (s *server) StreamUpdateWithMetadata(
	stream vald.UpdateWithMetadata_StreamUpdateWithMetadataServer,
) (err error) {
	ctx, span := trace.StartSpan(
		grpc.WithGRPCMethod(stream.Context(), vald.PackageName+"."+vald.UpdateRPCServiceName+"/"+vald.StreamUpdateRPCName+vald.MetadataSpanName),
		apiName+"/"+vald.StreamUpdateRPCName+vald.MetadataSpanName,
	)
	defer func() {
		if span != nil {
			span.End()
	...
	err = grpc.BidirectionalStream(ctx, stream, s.streamConcurrency,
		func(ctx context.Context, req *payload.Update_Request) (*payload.Object_StreamLocation, error) {
			...
			res, err := s.UpdateWithMetadata(ctx, req)
			if err != nil {
				st, _ := status.FromError(err)
				if st != nil && sspan != nil {
...
func (s *server) MultiUpdateWithMetadata(
	ctx context.Context, reqs *payload.Update_MultiRequest,
) (locs *payload.Object_Locations, errs error) {
	ctx, span := trace.StartSpan(
		grpc.WithGRPCMethod(ctx, vald.PackageName+"."+vald.UpdateRPCServiceName+"/"+vald.MultiUpdateRPCName+vald.MetadataSpanName),
		apiName+"/"+vald.MultiUpdateRPCName+vald.MetadataSpanName,
	)
	defer func() {
		if span != nil {
			span.End()
	...
	for i, r := range reqs.GetRequests() {
		if r != nil && r.GetVector() != nil && len(r.GetVector().GetVector()) >= algorithm.MinimumVectorDimensionSize && r.GetVector().GetId() != "" {
			...
			eg.Go(safety.RecoverFunc(func() (err error) {
				...
				res, err := s.UpdateWithMetadata(ectx, req)
				if err != nil {
					st, _ := status.FromError(err)
					if st != nil && sspan != nil {
						sspan.RecordError(err)
...
func (s *server) UpdateTimestampWithMetadata(
	...
	err = s.metadataClient.Put(ctx, []byte(req.Id), req.Metadata)
	if err != nil {
		st, _ := status.FromError(err)
		if st != nil && span != nil {
			span.RecordError(err)
