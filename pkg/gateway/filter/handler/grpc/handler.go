...
import (
	...
	"github.com/vdaas/vald/apis/grpc/v1/gateway/filter"
...
func (s *server) search(ctx context.Context,
	...
	if err != nil {
		...
		return nil, status.WrapWithInternal("search API failed to process search request", err, res, info.Get())
	}

	dists, err := s.filter.EgressVectors(ctx, res.Results)
	if err != nil {
		if span != nil {
			span.SetStatus(trace.StatusCodeInternal(err.Error()))
		}
		return nil, status.WrapWithInternal("search API failed to filter search response", err, res, info.Get())
	}
	res.Results = dists
	return res, nil
}
...
func (s *server) Insert(ctx context.Context, vec *payload.Object_Vector) (ce *payload.Object_Location, err error) {
	...
	v, err := s.filter.IngressInsertVector(ctx, vec.GetVector())
	if err != nil {
		if span != nil {
			span.SetStatus(trace.StatusCodeInternal(err.Error()))
		}
		return nil, status.WrapWithInternal("Insert API failed to filter insert request", err, vec, info.Get())
	}
	vec.Vector = v
