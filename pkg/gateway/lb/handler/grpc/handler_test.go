...
func Test_server_Exists(t *testing.T) {
	...
	defaultCheckFunc := func(w want, gotId *payload.Object_ID, err error) error {
		...
		if !reflect.DeepEqual(gotId, w.wantId) {
			return errors.Errorf("got = %v, want %v", gotId, w.wantId)
		}
		return nil
	}
	tests := []test{
	...
	for _, test := range tests {
		t.Run(test.name, func(tt *testing.T) {
			...
			if err := test.checkFunc(test.want, gotId, err); err != nil {
				tt.Errorf("error = %v", err)
			}
...
func Test_reStructureLocations(t *testing.T) {
	type args struct {
		uuids []string
	...
	for _, test := range tests {
		t.Run(test.name, func(tt *testing.T) {
			...
			got := reStructureLocations(test.args.uuids, test.args.locs)
			if err := test.checkFunc(test.want, got); err != nil {
				tt.Errorf("error = %v", err)
			}
