...
func TestVrsBuilder_Build(t *testing.T) {
	...
	itemData, err := yaml.Marshal(topItem)
	assert.NoError(t, err)
	...
	assert.NoError(t, yaml.Unmarshal(expected, &want))
	assert.NoError(t, yaml.Unmarshal(itemData, &got))
...
func newOverlayBuilder(raw []byte) *vrsBuilder {
	cr := &v1.ValdOperatorRelease{}
	cr.Spec.VectorEngine.Vald.Overlay = apiextensionsv1.JSON{Raw: raw}
...
func TestVrsBuilder_MakeOverlayPatch_ClonesParsedOverlay(t *testing.T) {
	...
	dim, found, err := unstructured.NestedFloat64(patch2.Object, "spec", "agent", "ngt", "dimension")
	assert.NoError(t, err)
