...
// newMergeOverlayRow returns a minimal ValdRelease with the GVK set, as Build
// sets it on every row before calling mergeOverlay.
func newMergeOverlayRow(name, namespace string) *valdrelease.ValdRelease {
	row := &valdrelease.ValdRelease{}
	row.SetGroupVersionKind(valdrelease.GVK)
	row.SetName(name)
	row.SetNamespace(namespace)
	row.Spec.Agent = &valdrelease.Agent{
		Ngt: &valdrelease.AgentNgt{Dimension: ptr(128)},
	}
	return row
}

func TestVrsBuilder_MergeOverlay(t *testing.T) {
	t.Run("base template fields survive when row and overlay omit them", func(t *testing.T) {
		cfg := initConfig(t)
		b := &vrsBuilder{cfg: cfg, gvk: valdrelease.GVK}

		merged, err := b.mergeOverlay(newMergeOverlayRow("cluster-a", "ns-a"))
		assert.NoError(t, err)
		assert.NotNil(t, merged.Spec.Agent.Hpa)
		assert.NotNil(t, merged.Spec.Agent.Hpa.Enabled)
		// testdata/vrs.yaml sets agent.hpa.enabled=false and the row/overlay
		// never touch it, so the base template value must come through.
		assert.False(t, *merged.Spec.Agent.Hpa.Enabled)
	})

	t.Run("row values win over the base template", func(t *testing.T) {
		cfg := initConfig(t)
		b := &vrsBuilder{cfg: cfg, gvk: valdrelease.GVK}

		merged, err := b.mergeOverlay(newMergeOverlayRow("cluster-a", "ns-a"))
		assert.NoError(t, err)
		// testdata/vrs.yaml sets agent.ngt.dimension=2; the row sets 128 and
		// must win.
		assert.NotNil(t, merged.Spec.Agent.Ngt.Dimension)
		assert.Equal(t, 128, *merged.Spec.Agent.Ngt.Dimension)
	})

	t.Run("overlay patch wins over the row", func(t *testing.T) {
		cfg := initConfig(t)
		b := &vrsBuilder{cfg: cfg, gvk: valdrelease.GVK}
		b.overlay = &k8s.Unstructured{Object: map[string]any{
			"spec": map[string]any{
				"agent": map[string]any{"ngt": map[string]any{"dimension": 256}},
			},
		}}
		...
		assert.NoError(t, err)
		assert.NotNil(t, merged.Spec.Agent.Ngt.Dimension)
		assert.Equal(t, 256, *merged.Spec.Agent.Ngt.Dimension, "the CR overlay must win over the built row")
	})

	t.Run("no overlay patch merges only base and row", func(t *testing.T) {
		cfg := initConfig(t)
		b := &vrsBuilder{cfg: cfg, gvk: valdrelease.GVK}
		assert.Nil(t, b.overlay)
	...
	t.Run("name and namespace are taken from the row", func(t *testing.T) {
		cfg := initConfig(t)
		b := &vrsBuilder{cfg: cfg, gvk: valdrelease.GVK}

		merged, err := b.mergeOverlay(newMergeOverlayRow("cluster-a", "ns-a"))
		assert.NoError(t, err)
		assert.Equal(t, "cluster-a", merged.GetName())
		assert.Equal(t, "ns-a", merged.GetNamespace())
	})

	t.Run("the cached default template is not mutated across calls", func(t *testing.T) {
		cfg := initConfig(t)
		b := &vrsBuilder{cfg: cfg, gvk: valdrelease.GVK}

		_, err := b.mergeOverlay(newMergeOverlayRow("cluster-1", "ns-1"))
		assert.NoError(t, err)

		merged2, err := b.mergeOverlay(newMergeOverlayRow("cluster-2", "ns-2"))
		assert.NoError(t, err)
		assert.Equal(t, "cluster-2", merged2.GetName())
		assert.Equal(t, "ns-2", merged2.GetNamespace())

		// The name/namespace set on the first call's baseVrs copy must not
		// have leaked back into the cached template used for the second call.
		assert.NotEqual(t, "cluster-1", b.cfg.DefaultVrs.Us.GetName())
	})

	t.Run("each call returns an independently mutable result", func(t *testing.T) {
		cfg := initConfig(t)
		b := &vrsBuilder{cfg: cfg, gvk: valdrelease.GVK}

		merged1, err := b.mergeOverlay(newMergeOverlayRow("cluster-1", "ns-1"))
		assert.NoError(t, err)
		*merged1.Spec.Agent.Ngt.Dimension = 999

		merged2, err := b.mergeOverlay(newMergeOverlayRow("cluster-2", "ns-2"))
		assert.NoError(t, err)
		assert.Equal(t, 128, *merged2.Spec.Agent.Ngt.Dimension,
			"mutating one merged result must not affect a later merge")
	})
}
