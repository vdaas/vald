...
// Operator represents an interface for the vald operator.
...
// New returns Operator object if no error occurs.
...
// Start starts the vald operator controller loop.
func (o *operator) Start(ctx context.Context) (<-chan error, error) {
	...
	ech := make(chan error, 2) //nolint:mnd
