...
// Name is the name of the ValdOperatorRelease resource controller.
...
// newResourceController returns the resource controller that reconciles
// ValdOperatorRelease resources with the given runtime configuration.
...
func (rc *resourceController) NewReconciler(
	_ context.Context, mgr manager.Manager,
...
// For returns the runtime.Object which is ValdOperatorRelease.
func (*resourceController) For() (k8s.Object, []builder.ForOption) {
	return new(v1.ValdOperatorRelease), nil
}

// Owns returns the ValdRelease owned by the ValdOperatorRelease.
func (*resourceController) Owns() (k8s.Object, []builder.OwnsOption) {
...
func (*resourceController) Watches() (k8s.Object, handler.EventHandler, []builder.WatchesOption) {
	return nil, nil, nil
}

// reconciler reconciles a ValdOperatorRelease object: it fetches the CR through the
// type-bound ObjectClient, walks the phase pipeline (build → sync → check)
...
func (r *reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		...
		if r.cfg.RequeueAfterNotFound > 0 {
			return ctrl.Result{RequeueAfter: r.cfg.RequeueAfterNotFound}, nil
		}
	...
	if err := r.reconcileValdOperatorRelease(ctx, cr); err != nil {
		if errors.Is(err, errPhaseWaiting) {
			...
			return ctrl.Result{RequeueAfter: r.waitingRequeueInterval()}, nil
		}
		log.Error(err)
		if r.cfg.RequeueAfterError > 0 {
			...
			return ctrl.Result{RequeueAfter: r.cfg.RequeueAfterError}, nil
		}
		return ctrl.Result{}, err
	}

	if r.cfg.RequeueAfterSuccess > 0 {
		return ctrl.Result{RequeueAfter: r.cfg.RequeueAfterSuccess}, nil
	}
	return ctrl.Result{}, nil
}
...
func (r *reconciler) reconcileValdOperatorRelease(
	...
	defer func() {
		if !equality.Semantic.DeepEqual(originalStatus, &cr.Status) {
...
func (r *reconciler) reconcilePhase(
	...
	if cn.Status == metav1.ConditionUnknown {
...
// waitingRequeueInterval returns the requeue interval applied to waiting
// (Progressing/Pending) phases: requeue.success when configured, otherwise
// the fixed defaultRequeueAfterWaiting.
...
// syncPhase builds the desired objects of the phase and hands them to the
// generic syncer, which applies them with owner references and prunes the
// stale ones fetched by the phase.
...
// advance moves Status.Phase / Progress.Completed to the next phase in the
// pipeline if one exists. The seeded condition is Succeeded for terminal
// phases and Progressing otherwise. Returns the index of the new current
// phase, or -1 if no next phase existed.
func (*reconciler) advance(ps phases, current int, cr *v1.ValdOperatorRelease) int {
	...
	meta.SetStatusCondition(&cr.Status.Conditions, p.condition(seed))
	return next
}
...
// checkClusters reports whether every cluster referenced by the CR
// infrastructure has been provisioned (both ID and Name assigned by the
// external system).
