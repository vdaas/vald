...
type Data struct {
	...
	// Operator represent vald operator configuration
...
// LeaderElection represents the configurations for the manager leader election.
...
// Controller represents the configurations for the controller manager and the
...
// VrsGateway represents the gateway-related values the operator injects into
// the generated ValdRelease.
...
// VrsIndexer represents the index-manager values the operator injects into
// the generated ValdRelease.
...
type Vrs struct {
	...
	// InternalHostDomain represents the internal host domain of the cluster.
	InternalHostDomain string `json:"internal_host_domain" yaml:"internal_host_domain"`
...
// PersistentVolume represents the fallback and sizing configurations for
...
// Operator represents the configurations for the vald operator.
...
func (o *Operator) bindVrs() {
	...
	v.InternalHostDomain = config.GetActualValue(v.InternalHostDomain)
...
// DefaultVrsBundle bundles the template ValdRelease yaml: source path, raw
// bytes, and the parsed unstructured form used by Build's overlay merge.
...
func (o *Operator) Load() (*Config, error) {
	...
	c := &Config{
		InternalHostDomain:   o.Vrs.InternalHostDomain,
