...
// updateCachedStatuses walks desired and, for every name that resolve maps
// to a changed cached object (resolve applies the new status and returns the
// object to persist, or ok=false to skip), issues a status update and
// collects the names actually updated. Failed updates are logged and skipped,
// matching the previous per-resource loops this helper replaces. Resolution
// stays with the caller because the cached shapes differ (scenario wrapper
// exposing Crd vs raw job object).
...
func (o *operator) updateBenchmarkJobStatus(
	...
	return updateCachedStatuses(ctx, o.k8sClient(), js,
