...
func (e *export) doExportIndex(
	...
	stream, err := vc.NewValdClient(conn).StreamListObject(ctx, emptyReq, nil)
	if err != nil || stream == nil {
		return err
	}
	log.Infof("starting exporter to %s", gatewayAddrs[0])
		...
		default:
			...
			if err != nil {
				...
				eg.Go(safety.RecoverFunc(func() (err error) {
					...
					storedBinary, ok, err := e.storedVector.Get(id)
					if err != nil {
					...
					isUpsertVector := !ok || storedObjVec.GetTimestamp() < ts
					if isUpsertVector {
						protoData, err := objVec.MarshalVT()
						if err != nil {
							panic(err)
						}
						e.storedVector.Set(id, protoData)
					}
					return nil
				}))
			}
