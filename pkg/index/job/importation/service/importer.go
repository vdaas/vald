...
func (i *importer) doImportIndex(
	...
	stream, err := vcClient.StreamInsert(ctx, grpcCallOpts...)
	...
	if err := i.storedVector.Range(ctx, func(key string, value []byte) bool {
		eg.Go(safety.RecoverFunc(func() (err error) {
			log.Info("sending object vector to stream", key)
			objVec := new(payload.Object_Vector)
			if err := objVec.UnmarshalVT(value); err != nil {
				log.Errorf("failed to Unmarshal proto to payload.Object_Vector: %v",
					err)
				return err
			}

			if err := stream.Send(&payload.Insert_Request{
				Vector: objVec,
				Config: &payload.Insert_Config{
					SkipStrictExistCheck: !i.forceUpdate,
				},
			}); err != nil {
				log.Errorf("failed to send object vector to stream: %v", err)
				return err
			}
			return nil
		}))
		...
		log.Errorf("failed to Range from check list: %v", err)
		return err
	}
