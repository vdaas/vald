...
func Test_handler_IndexInfo(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotCode, err := tt.h.IndexInfo(tt.args.w, tt.args.r)
			if (err != nil) != tt.wantErr {
				t.Errorf("handler.IndexInfo() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if gotCode != tt.wantCode {
				t.Errorf("handler.IndexInfo() = %v, want %v", gotCode, tt.wantCode)
			}
		})
	}
}
