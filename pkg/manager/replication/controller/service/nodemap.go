...
type readOnlyNodeMap struct {
	m       map[string]*entryNodeMap
...
func (m *nodeMap) Store(key string, value node.Node) {
	...
	if e, ok := read.m[key]; ok {
		...
		if !read.amended {
			...
			m.dirtyLocked()
			m.read.Store(readOnlyNodeMap{m: read.m, amended: true})
		}
		m.dirty[key] = newEntryNodeMap(value)
	}
	m.mu.Unlock()
}
...
func (e *entryNodeMap) unexpungeLocked() (wasExpunged bool) {
	return atomic.CompareAndSwapPointer(&e.p, expungedNodeMap, nil)
}
...
func (m *nodeMap) Range(f func(key string, value node.Node) bool) {
	...
	if read.amended {
		...
		read, _ = m.read.Load().(readOnlyNodeMap)
		if read.amended {
			read = readOnlyNodeMap{m: m.dirty}
			m.read.Store(read)
			m.dirty = nil
			m.misses = 0
		}
...
func (m *nodeMap) missLocked() {
	...
	m.read.Store(readOnlyNodeMap{m: m.dirty})
	m.dirty = nil
	m.misses = 0
}
...
func (m *nodeMap) dirtyLocked() {
	...
	read, _ := m.read.Load().(readOnlyNodeMap)
	m.dirty = make(map[string]*entryNodeMap, len(read.m))
	for k, e := range read.m {
...
func (e *entryNodeMap) tryExpungeLocked() (isExpunged bool) {
	p := atomic.LoadPointer(&e.p)
	for p == nil {
		if atomic.CompareAndSwapPointer(&e.p, nil, expungedNodeMap) {
			return true
		}
		p = atomic.LoadPointer(&e.p)
	}
	return p == expungedNodeMap
}
