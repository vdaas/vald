...
type qbgState struct {
	states  map[string]vectorState
	vectors map[string]int
}

func (st *qbgState) Reset() {
...
var (
	insertACommand = &commands.ProtoCommand{
		...
		RunFunc: func(
			...
			sy := systemUnderTest.(*qbgSystem)
			qbg := sy.qbg

			err := qbg.Insert(idA, vectors[idA][0])
	...
	insertBCommand = &commands.ProtoCommand{
		...
		RunFunc: func(
			...
			err := qbg.Insert(idB, vectors[idB][0])
			return &resultContainer{
				err: err,
			}
		},
	...
	insertCCommand = &commands.ProtoCommand{
		...
		RunFunc: func(
			...
			err := qbg.Insert(idC, vectors[idC][0])
			return &resultContainer{
				err: err,
			}
		},
	...
	createIndexCommand = &commands.ProtoCommand{
		...
		RunFunc: func(
			...
			ctx := systemUnderTest.(*qbgSystem).ctx
			qbg := systemUnderTest.(*qbgSystem).qbg

			// WARN: dirty workaround
			// without the sleep before/after CreateIndex the tests usually fail
			time.Sleep(time.Second)

			err := qbg.CreateAndSaveIndex(ctx, 10000)
	...
	searchCommand = &commands.ProtoCommand{
		...
		RunFunc: func(
			...
			qbgSys := systemUnderTest.(*qbgSystem)
			qbg := qbgSys.qbg

			res, err := qbg.Search(qbgSys.ctx, []float32{0.1, 0.1, 0.1}, 3, 0.1, -1.0)
			return &resultContainer{
				err:     err,
				results: res,
			}
	...
	searchByIDACommand = &commands.ProtoCommand{
		...
		RunFunc: func(
			...
			_, res, err := qbg.SearchByID(qbgSys.ctx, idA, 3, 0.1, -1.0)
			return &resultContainer{
				err:     err,
				results: res,
			}
	...
	searchByIDBCommand = &commands.ProtoCommand{
		...
		RunFunc: func(
			...
			_, res, err := qbg.SearchByID(qbgSys.ctx, idB, 3, 0.1, -1.0)
			return &resultContainer{
				err:     err,
				results: res,
			}
	...
	searchByIDCCommand = &commands.ProtoCommand{
		...
		RunFunc: func(
			...
			_, res, err := qbg.SearchByID(qbgSys.ctx, idC, 3, 0.1, -1.0)
			return &resultContainer{
				err:     err,
				results: res,
			}
	...
	updateACommand = &commands.ProtoCommand{
		...
		RunFunc: func(
			...
			err := qbg.Update(idA, vectors[idA][1])
			return &resultContainer{
				err: err,
			}
		},
	...
	updateBCommand = &commands.ProtoCommand{
		...
		RunFunc: func(
			...
			err := qbg.Update(idB, vectors[idB][1])
			return &resultContainer{
				err: err,
			}
		},
	...
	updateCCommand = &commands.ProtoCommand{
		...
		RunFunc: func(
			...
			err := qbg.Update(idC, vectors[idC][1])
			return &resultContainer{
				err: err,
			}
		},
...
func rootCommands(t *testing.T) commands.Commands {
	...
	return &commands.ProtoCommands{
		...
		InitialStateGen: gen.Const(&qbgState{
			states: map[string]vectorState{
				idA: NOT_INSERTED,
				idB: NOT_INSERTED,
				idC: NOT_INSERTED,
...
func TestStatefulQBG(t *testing.T) {
	// initialize logger
	if applicationLog {
		log.Init()
	} else {
	...
	properties.Property("QBG", commands.Prop(rootCommands(t)))
