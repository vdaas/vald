...
func TestWithAutoIndexLeqbgh(t *testing.T) {
	type T = qbg
	type args struct {
		l int
	}
	...
	tests := []test{
		...
		{
			name: "set index leqbgh success when leqbgh is 0",
			args: args{
				l: 0,
			},
			want: want{
	...
	for _, tc := range tests {
		...
		t.Run(test.name, func(tt *testing.T) {
			...
			got := WithAutoIndexLeqbgh(test.args.l)
			obj := new(T)
			if err := checkFunc(test.want, obj, got(obj)); err != nil {
				tt.Errorf("error = %v", err)
			}
