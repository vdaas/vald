...
import (
	...
	"github.com/vdaas/vald/pkg/agent/core/qbg/service/kvs"
	"github.com/vdaas/vald/pkg/agent/core/qbg/service/vqueue"
...
const (
	kvsFileName          = "qbg-meta.kvsdb"
	kvsTimestampFileName = "qbg-timestamp.kvsdb"
	noTimeStampFile      = -1
...
func New(cfg *config.QBG, opts ...Option) (nn QBG, err error) {
	n := &qbg{
...
func (n *qbg) prepareFolders(ctx context.Context) (err error) {
...
func (n *qbg) load(ctx context.Context, path string, opts ...core.Option) (err error) {
	...
	if err != nil && errors.Is(err, fs.ErrNotExist) || agentMetadata == nil || agentMetadata.QBG == nil || agentMetadata.QBG.IndexCount == 0 {
	...
	if agentMetadata != nil && agentMetadata.QBG != nil {
		log.Debugf("the backup index size is %d. starting to load...", agentMetadata.QBG.IndexCount)
		timeout = time.Duration(
			math.Min(
				math.Max(
					float64(agentMetadata.QBG.IndexCount)*float64(n.litFactor),
					float64(n.minLit),
				),
				float64(n.maxLit),
			),
	...
	eg.Go(safety.RecoverFunc(func() (err error) {
		...
		if err != nil {
			err = errors.Wrapf(err, "failed to load qbg index from path: %s", path)
			return err
		}
		return nil
	}))
	...
	eg.Go(safety.RecoverFunc(func() (err error) {
		...
		if n.kvs != nil && float64(agentMetadata.QBG.IndexCount/2) > float64(n.kvs.Len()) {
...
func (n *qbg) backupBroken(ctx context.Context) error {
	if n.historyLimit <= 0 {
		return nil
	}
...
func needsBackup(backupPath string) bool {
	...
	if meta.IsInvalid || meta.QBG.IndexCount > 0 {
		return true
	}
...
func (n *qbg) rebuild(ctx context.Context, path string, opts ...core.Option) (err error) {
...
func (n *qbg) initQBG(opts ...core.Option) (err error) {
	if n.kvs == nil {
...
func (n *qbg) loadKVS(ctx context.Context, path string, timeout time.Duration) (err error) {
	ctx, cancel := context.WithTimeout(ctx, timeout)
	defer cancel()
	eg, _ := errgroup.New(ctx)
	...
	for k, id := range m {
		if ts, ok := mt[k]; ok {
			...
			// NOTE: SaveIndex do not write qbg-timestamp.kvsdb with timestamp 0.
			n.kvs.Set(k, id, 0)
			n.fmap[k] = int64(id)
		}
	}
...
func (n *qbg) Start(ctx context.Context) <-chan error {
	if n.dcd {
		return nil
	}
...
func (n *qbg) Search(ctx context.Context, vec []float32, size uint32, epsilon, radius float32) (res *payload.Search_Response, err error) {
	if n.IsIndexing() {
	...
	if err != nil {
		...
		log.Errorf("cgo error detected during search: qbg api returned error %v", err)
		return nil, err
	}
...
func (n *qbg) SearchByID(ctx context.Context, uuid string, size uint32, epsilon, radius float32) (vec []float32, dst *payload.Search_Response, err error) {
	if n.IsIndexing() {
...
func (n *qbg) LinearSearch(ctx context.Context, vec []float32, size uint32) (res *payload.Search_Response, err error) {
	if n.IsIndexing() {
		return nil, errors.ErrCreateIndexingIsInProgress
	}
	...
	if err != nil {
		...
		log.Errorf("cgo error detected during linear search: qbg api returned error %v", err)
		return nil, err
	}
...
func (n *qbg) LinearSearchByID(ctx context.Context, uuid string, size uint32) (vec []float32, dst *payload.Search_Response, err error) {
	if n.IsIndexing() {
		return nil, nil, errors.ErrCreateIndexingIsInProgress
	}
	vec, _, err = n.GetObject(uuid)
...
func (n *qbg) Insert(uuid string, vec []float32) (err error) {
	return n.insert(uuid, vec, time.Now().UnixNano(), true)
}

func (n *qbg) InsertWithTime(uuid string, vec []float32, t int64) (err error) {
	if t <= 0 {
		t = time.Now().UnixNano()
	}
...
func (n *qbg) insert(uuid string, vec []float32, t int64, validation bool) (err error) {
	if len(uuid) == 0 {
...
func (n *qbg) InsertMultiple(vecs map[string][]float32) (err error) {
	return n.insertMultiple(vecs, time.Now().UnixNano(), true)
}

func (n *qbg) InsertMultipleWithTime(vecs map[string][]float32, t int64) (err error) {
	if t <= 0 {
		t = time.Now().UnixNano()
	}
...
func (n *qbg) insertMultiple(vecs map[string][]float32, now int64, validation bool) (err error) {
...
func (n *qbg) Update(uuid string, vec []float32) (err error) {
	return n.update(uuid, vec, time.Now().UnixNano())
}

func (n *qbg) UpdateWithTime(uuid string, vec []float32, t int64) (err error) {
	if t <= 0 {
		t = time.Now().UnixNano()
	}
...
func (n *qbg) update(uuid string, vec []float32, t int64) (err error) {
...
func (n *qbg) UpdateMultiple(vecs map[string][]float32) (err error) {
	return n.updateMultiple(vecs, time.Now().UnixNano())
}

func (n *qbg) UpdateMultipleWithTime(vecs map[string][]float32, t int64) (err error) {
	if t <= 0 {
		t = time.Now().UnixNano()
	}
...
func (n *qbg) updateMultiple(vecs map[string][]float32, t int64) (err error) {
...
func (n *qbg) Delete(uuid string) (err error) {
	return n.delete(uuid, time.Now().UnixNano(), true)
}

func (n *qbg) DeleteWithTime(uuid string, t int64) (err error) {
	if t <= 0 {
		t = time.Now().UnixNano()
	}
...
func (n *qbg) delete(uuid string, t int64, validation bool) (err error) {
	if len(uuid) == 0 {
		err = errors.ErrUUIDNotFound(0)
		return err
	}
...
func (n *qbg) DeleteMultiple(uuids ...string) (err error) {
	return n.deleteMultiple(uuids, time.Now().UnixNano(), true)
}

func (n *qbg) DeleteMultipleWithTime(uuids []string, t int64) (err error) {
	if t <= 0 {
		t = time.Now().UnixNano()
	}
...
func (n *qbg) deleteMultiple(uuids []string, now int64, validation bool) (err error) {
...
func (n *qbg) CreateIndex(ctx context.Context, poolSize uint32) (err error) {
	ctx, span := trace.StartSpan(ctx, "vald/agent-qbg/service/QBG.CreateIndex")
	defer func() {
		if span != nil {
			span.End()
		}
	...
	n.vq.RangePopDelete(ctx, now, func(uuid string) bool {
		...
		log.Debugf("start remove operation for qbg index id: %s, oid: %d", uuid, oid)
		if err := n.core.Remove(uint(oid)); err != nil {
			log.Errorf("failed to remove oid: %d from qbg index. error: %v", oid, err)
			n.fmu.Lock()
			n.fmap[uuid] = int64(oid)
			n.fmu.Unlock()
		}
		log.Debugf("removed from qbg index and kvsdb id: %s, oid: %d", uuid, oid)
		return true
	})
	...
	n.vq.RangePopInsert(ctx, now, func(uuid string, vector []float32, timestamp int64) bool {
		log.Debugf("start insert operation for qbg index id: %s", uuid)
		oid, err := n.core.Insert(vector)
		if err != nil {
			log.Warnf("failed to insert vector uuid: %s vec: %v to qbg index. error: %v", uuid, vector, err)
			...
			oid, err = n.core.Insert(vector)
			if err != nil {
				log.Errorf("failed to retry insert vector uuid: %s vec: %v to qbg index. error: %v", uuid, vector, err)
				return true
			}
		}
		...
		n.fmu.Unlock()
		log.Debugf("finished to insert qbg index and kvsdb id: %s, oid: %d", uuid, oid)
		return true
	})
	if poolSize <= 0 {
...
func (n *qbg) removeInvalidIndex(ctx context.Context) {
	if n.kvs.Len() == 0 {
		return
	}
	var dcnt uint32
...
func (n *qbg) SaveIndex(ctx context.Context) (err error) {
	ctx, span := trace.StartSpan(ctx, "vald/agent-qbg/service/QBG.SaveIndex")
	defer func() {
		if span != nil {
			span.End()
		}
...
func (n *qbg) CreateAndSaveIndex(ctx context.Context, poolSize uint32) (err error) {
	ctx, span := trace.StartSpan(ctx, "vald/agent-qbg/service/QBG.CreateAndSaveIndex")
	defer func() {
		if span != nil {
			span.End()
		}
...
func (n *qbg) moveAndSwitchSavedData(ctx context.Context) (err error) {
	if !n.enableCopyOnWrite {
		return nil
	}
	n.cowmu.Lock()
...
func (n *qbg) Exists(uuid string) (oid uint32, ok bool) {
...
func (n *qbg) GetObject(uuid string) (vec []float32, timestamp int64, err error) {
	...
	vec, err = n.core.GetVector(uint(oid))
	if err != nil {
		log.Debugf("GetObject\tuuid: %s oid: %d's vector not found in qbg index", uuid, oid)
...
func (n *qbg) readyForUpdate(uuid string, vec []float32) (err error) {
	if len(uuid) == 0 {
	...
	// if vector leqbgh is not equal or if some difference exists let's try update
...
func (n *qbg) IsSaving() bool {
	s, ok := n.saving.Load().(bool)
	return s && ok
}

func (n *qbg) IsIndexing() bool {
	i, ok := n.indexing.Load().(bool)
	return i && ok
}

func (n *qbg) UUIDs(ctx context.Context) (uuids []string) {
...
func (n *qbg) NumberOfCreateIndexExecution() uint64 {
	return atomic.LoadUint64(&n.nocie)
}

func (n *qbg) NumberOfProactiveGCExecution() uint64 {
...
func (n *qbg) Len() uint64 {
	return n.kvs.Len()
}

func (n *qbg) InsertVQueueBufferLen() uint64 {
	return uint64(n.vq.IVQLen())
}

func (n *qbg) DeleteVQueueBufferLen() uint64 {
	return uint64(n.vq.DVQLen())
}

func (n *qbg) GetDimensionSize() int {
	return n.dim
}

func (n *qbg) Close(ctx context.Context) (err error) {
	err = n.kvs.Close()
	if len(n.path) != 0 {
...
func (n *qbg) BrokenIndexCount() uint64 {
...
func (n *qbg) ListObjectFunc(ctx context.Context, f func(uuid string, oid uint32, ts int64) bool) {
...
func (n *qbg) toSearchResponse(sr []core.SearchResult) (res *payload.Search_Response, err error) {
	if len(sr) == 0 {
		if n.Len() == 0 {
			return nil, nil
		}
