...
func (s *server) Search(ctx context.Context, req *payload.Search_Request) (res *payload.Search_Response, err error) {
	...
	if len(req.GetVector()) != s.qbg.GetDimensionSize() {
		...
		err = status.WrapWithInvalidArgument("Search API Incompatible Dimension Size detected",
			...
			&errdetails.ResourceInfo{
				ResourceType: qbgResourceType + "/qbg.Search",
			})
		log.Warn(err)
		if span != nil {
	...
	if err != nil || res == nil {
		...
		case errors.As(err, &errQBG):
			...
			err = status.WrapWithInternal("Search API failed to process search request due to qbg core process returned error", err,
				&errdetails.RequestInfo{
					RequestId:   req.GetConfig().GetRequestId(),
					ServingData: errdetails.Serialize(req),
				},
				&errdetails.ResourceInfo{
					ResourceType: qbgResourceType + "/qbg.Search/core.qbg",
					ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, s.name, s.ip),
				}, info.Get())
			log.Error(err)
...
func (s *server) SearchByID(ctx context.Context, req *payload.Search_IDRequest) (res *payload.Search_Response, err error) {
	...
	if len(uuid) == 0 {
		...
		err = status.WrapWithInvalidArgument(fmt.Sprintf("SearchByID API invalid argument for uuid \"%s\" detected", uuid), err,
			...
			&errdetails.ResourceInfo{
				ResourceType: qbgResourceType + "/qbg.SearchByID",
				ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, s.name, s.ip),
			})
		log.Warn(err)
		if span != nil {
	...
	vec, res, err := s.qbg.SearchByID(ctx,
		uuid,
		req.GetConfig().GetNum(),
		req.GetConfig().GetEpsilon(),
		req.GetConfig().GetRadius())
	...
	if err != nil || res == nil {
		...
		case errors.As(err, &errQBG):
			log.Errorf("qbg core process returned error: %v", err)
			err = status.WrapWithInternal("SearchByID API failed to process search request due to qbg core process returned error", err,
				&errdetails.RequestInfo{
					RequestId:   req.GetConfig().GetRequestId(),
					ServingData: errdetails.Serialize(req),
				},
				&errdetails.ResourceInfo{
					ResourceType: qbgResourceType + "/qbg.SearchByID/core.qbg",
					ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, s.name, s.ip),
				}, info.Get())
			log.Error(err)
			attrs = trace.StatusCodeInternal(err.Error())
...
func (s *server) MultiSearch(ctx context.Context, reqs *payload.Search_MultiRequest) (res *payload.Search_Responses, errs error) {
	...
	if errs != nil {
		st, msg, err := status.ParseError(errs, codes.Internal,
			...
			&errdetails.ResourceInfo{
				ResourceType: qbgResourceType + "/qbg.MultiSearch",
				ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, s.name, s.ip),
			})
		if span != nil {
			span.RecordError(err)
...
func (s *server) MultiSearchByID(ctx context.Context, reqs *payload.Search_MultiIDRequest) (res *payload.Search_Responses, errs error) {
	...
	if errs != nil {
		st, msg, err := status.ParseError(errs, codes.Internal,
			...
			&errdetails.ResourceInfo{
				ResourceType: qbgResourceType + "/qbg.MultiSearchByID",
				ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, s.name, s.ip),
			})
		if span != nil {
			span.RecordError(err)
