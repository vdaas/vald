...
func Test_server_Upsert(t *testing.T) {
	...
	defaultBeforeFunc := func(objectType string, insertNum int) func(*testing.T, context.Context, optIdx) (Server, error) {
		...
		return func(t *testing.T, ctx context.Context, opt optIdx) (Server, error) {
			...
			qbg, err := newIndexedQBGService(ctx, eg, request.Float, vector.Gaussian, insertNum, defaultInsertConfig, cfg, nil, overwriteID, overwriteVec)
			if err != nil {
				return nil, err
			}
	...
	*/
	tests := []test{
		...
		},
		{
			...
			beforeFunc: defaultBeforeFunc(qbg.Uint8.String(), defaultInsertNum),
		},
		{
		...
		},
		{
			...
			beforeFunc: defaultBeforeFunc(qbg.Float.String(), 0),
			want: want{
				code: codes.InvalidArgument,
			},
		},
		...
		},
		{
			...
			beforeFunc: defaultBeforeFunc(qbg.Uint8.String(), 1000),
			want: want{
				wantUUID: "test",
			},
		},
		...
		},
		{
			...
			beforeFunc: defaultBeforeFunc(qbg.Uint8.String(), 0),
			want: want{
				wantUUID: "test",
			},
		},
	...
	for _, tc := range tests {
		...
		t.Run(test.name, func(tt *testing.T) {
			...
			if test.beforeFunc == nil {
				test.beforeFunc = defaultBeforeFunc(qbg.Float.String(), defaultInsertNum)
			}
