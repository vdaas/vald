...
func (s *server) Update(ctx context.Context, req *payload.Update_Request) (res *payload.Object_Location, err error) {
	...
	vec := req.GetVector()
	if len(vec.GetVector()) != s.qbg.GetDimensionSize() {
		err = errors.ErrIncompatibleDimensionSize(len(vec.GetVector()), int(s.qbg.GetDimensionSize()))
		err = status.WrapWithInvalidArgument("Update API Incompatible Dimension Size detected",
			...
			&errdetails.ResourceInfo{
				ResourceType: qbgResourceType + "/qbg.Update",
	...
	err = s.qbg.UpdateWithTime(uuid, vec.GetVector(), req.GetConfig().GetTimestamp())
	if err != nil {
		...
		} else if errors.Is(err, errors.ErrUUIDNotFound(0)) || errors.Is(err, errors.ErrInvalidDimensionSize(len(vec.GetVector()), s.qbg.GetDimensionSize())) {
	...
	for _, req := range reqs.GetRequests() {
		...
		if len(vec.GetVector()) != s.qbg.GetDimensionSize() {
			...
			err = status.WrapWithInvalidArgument("MultiUpdate API Incompatible Dimension Size detected",
				...
				&errdetails.ResourceInfo{
					ResourceType: qbgResourceType + "/qbg.MultiUpdate",
					ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, s.name, s.ip),
				})
			log.Warn(err)
			if span != nil {
	...
	if err != nil {
		...
		if notFoundIDs := func() []string {
			...
			for id, vec := range vmap {
				if errors.Is(err, errors.ErrInvalidDimensionSize(len(vec), s.qbg.GetDimensionSize())) {
					idis = append(idis, id)
				}
			}
			return idis
