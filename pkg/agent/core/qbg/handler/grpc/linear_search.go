...
func (s *server) LinearSearch(ctx context.Context, req *payload.Search_Request) (res *payload.Search_Response, err error) {
	...
	if len(req.GetVector()) != s.qbg.GetDimensionSize() {
		err = errors.ErrIncompatibleDimensionSize(len(req.GetVector()), int(s.qbg.GetDimensionSize()))
		err = status.WrapWithInvalidArgument("LinearSearch API Incompatible Dimension Size detected",
			...
			&errdetails.ResourceInfo{
				ResourceType: qbgResourceType + "/qbg.LinearSearch",
			})
		log.Warn(err)
		if span != nil {
	...
	res, err = s.qbg.LinearSearch(ctx,
		req.GetVector(),
	...
	if err != nil || res == nil {
		...
		case errors.As(err, &errQBG):
			log.Errorf("qbg core process returned error: %v", err)
			err = status.WrapWithInternal("LinearSearch API failed to process search request due to qbg core process returned error", err,
				&errdetails.RequestInfo{
					RequestId:   req.GetConfig().GetRequestId(),
					ServingData: errdetails.Serialize(req),
				},
				&errdetails.ResourceInfo{
					ResourceType: qbgResourceType + "/qbg.LinearSearch/core.qbg",
					ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, s.name, s.ip),
				}, info.Get())
			log.Error(err)
			attrs = trace.StatusCodeInternal(err.Error())
		case errors.Is(err, errors.ErrIncompatibleDimensionSize(len(req.GetVector()), int(s.qbg.GetDimensionSize()))):
			err = status.WrapWithInvalidArgument("LinearSearch API Incompatible Dimension Size detected",
				err,
				&errdetails.RequestInfo{
					RequestId:   req.GetConfig().GetRequestId(),
...
func (s *server) LinearSearchByID(ctx context.Context, req *payload.Search_IDRequest) (res *payload.Search_Response, err error) {
	...
	if len(uuid) == 0 {
		...
		err = status.WrapWithInvalidArgument(fmt.Sprintf("LinearSearchByID API invalid argument for uuid \"%s\" detected", uuid), err,
			...
			&errdetails.ResourceInfo{
				ResourceType: qbgResourceType + "/qbg.LinearSearchByID",
				ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, s.name, s.ip),
			})
		log.Warn(err)
		if span != nil {
	...
	vec, res, err := s.qbg.LinearSearchByID(ctx,
		uuid,
		req.GetConfig().GetNum())
	if err == nil && res == nil {
		return nil, nil
	...
	if err != nil || res == nil {
		...
		case errors.Is(err, errors.ErrObjectIDNotFound(req.GetId())),
			strings.Contains(err.Error(), fmt.Sprintf("qbg uuid %s's object not found", req.GetId())):
		...
		case errors.As(err, &errQBG):
			log.Errorf("qbg core process returned error: %v", err)
			err = status.WrapWithInternal("LinearSearchByID API failed to process search request due to qbg core process returned error", err,
				&errdetails.RequestInfo{
					RequestId:   req.GetConfig().GetRequestId(),
					ServingData: errdetails.Serialize(req),
				},
				&errdetails.ResourceInfo{
					ResourceType: qbgResourceType + "/qbg.LinearSearchByID/core.qbg",
					ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, s.name, s.ip),
				}, info.Get())
			log.Error(err)
			attrs = trace.StatusCodeInternal(err.Error())
		case errors.Is(err, errors.ErrIncompatibleDimensionSize(len(vec), int(s.qbg.GetDimensionSize()))):
...
func (s *server) MultiLinearSearch(ctx context.Context, reqs *payload.Search_MultiRequest) (res *payload.Search_Responses, errs error) {
	...
	if errs != nil {
		st, msg, err := status.ParseError(errs, codes.Internal,
			...
			&errdetails.ResourceInfo{
				ResourceType: qbgResourceType + "/qbg.MultiLinearSearch",
				ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, s.name, s.ip),
			})
		if span != nil {
			span.RecordError(err)
...
func (s *server) MultiLinearSearchByID(ctx context.Context, reqs *payload.Search_MultiIDRequest) (res *payload.Search_Responses, errs error) {
	...
	if errs != nil {
		st, msg, err := status.ParseError(errs, codes.Internal,
			...
			&errdetails.ResourceInfo{
				ResourceType: qbgResourceType + "/qbg.MultiLinearSearchByID",
				ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, s.name, s.ip),
			})
		if span != nil {
			span.RecordError(err)
