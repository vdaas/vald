...
func Test_server_Search(t *testing.T) {
	...
	defaultBeforeFunc := func(t *testing.T, ctx context.Context, f fields, a args) (Server, error) {
		...
		qbg, err := newIndexedQBGService(ctx, eg, f.objectType, f.distribution, a.insertNum, defaultInsertConfig, f.qbgCfg, f.qbgOpts, nil, f.overwriteVec)
		if err != nil {
			return nil, err
		}
	...
	qbgConfig := func(dim int, objectType string) *config.QBG {
		return &config.QBG{
			Dimension:        dim,
	...
	/*
		...
		- Boundary Value Testing
			...
			- case 7.1: fail search with 0 leqbgh vector from 1000 vectors (type: uint8)
			- case 7.2: fail search with 0 leqbgh vector from 1000 vectors (type: float32)
	...
	tests := []test{
		// Equivalence Class Testing
		{
			...
			fields: fields{
				...
				qbgCfg:       qbgConfig(defaultDimensionSize, qbg.Uint8.String()),
			},
			want: want{
		...
		},
		{
			...
			fields: fields{
				...
				qbgCfg:       qbgConfig(defaultDimensionSize, qbg.Float.String()),
			},
			want: want{
				resultSize: int(defaultSearch_Config.GetNum()),
			},
		...
		{
			name: "Boundary Value Testing case 7.1: fail search with 0 leqbgh vector (type: uint8)",
			args: args{
				insertNum: 1000,
				req: &payload.Search_Request{
					Vector: []float32{},
			...
			fields: fields{
				distribution: vector.Gaussian,
				objectType:   request.Uint8,
				qbgCfg:       qbgConfig(defaultDimensionSize, "uint8"),
			},
			want: want{
				resultSize: 0,
				code:       codes.InvalidArgument,
		...
		{
			name: "Boundary Value Testing case 7.2: fail search with 0 leqbgh vector (type: float32)",
			args: args{
				insertNum: 1000,
				req: &payload.Search_Request{
					Vector: []float32{},
