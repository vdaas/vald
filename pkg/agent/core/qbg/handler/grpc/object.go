	...
	if len(uuid) == 0 {
		...
		err = status.WrapWithInvalidArgument(fmt.Sprintf("Exists API invalid argument for uuid \"%s\" detected", uuid), err,
			...
			&errdetails.ResourceInfo{
				ResourceType: qbgResourceType + "/qbg.Exists",
...
func (s *server) GetObject(ctx context.Context, id *payload.Object_VectorRequest) (res *payload.Object_Vector, err error) {
	...
	if len(uuid) == 0 {
		...
		err = status.WrapWithInvalidArgument(fmt.Sprintf("GetObject API invalid argument for uuid \"%s\" detected", uuid), err,
			...
			&errdetails.ResourceInfo{
				ResourceType: qbgResourceType + "/qbg.GetObject",
				ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, s.name, s.ip),
			})
		log.Warn(err)
		if span != nil {
	...
	vec, ts, err := s.qbg.GetObject(uuid)
	if err != nil || vec == nil {
...
func (s *server) StreamListObject(_ *payload.Object_List_Request, stream vald.Object_StreamListObjectServer) (err error) {
	...
	var (
		mu   sync.Mutex
		emu  sync.RWMutex
		errs = make([]error, 0, s.qbg.Len())
		emap = make(map[string]struct{})
	)
	s.qbg.ListObjectFunc(ctx, func(uuid string, _ uint32, _ int64) bool {
		vec, ts, err := s.qbg.GetObject(uuid)
...
func (s *server) GetTimestamp(ctx context.Context, id *payload.Object_GetTimestampRequest) (res *payload.Object_Timestamp, err error) {
	...
	if len(uuid) == 0 {
		...
		err = status.WrapWithInvalidArgument(fmt.Sprintf("GetTimestamp API invalid argument for uuid \"%s\" detected", uuid), err,
			...
			&errdetails.ResourceInfo{
				ResourceType: qbgResourceType + "/qbg.GetTimestamp",
				ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, s.name, s.ip),
			})
		log.Warn(err)
		if span != nil {
