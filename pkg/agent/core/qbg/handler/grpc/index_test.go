		...
		},
		{
			...
			beforeFunc: func(t *testing.T, ctx context.Context, a args, s Server) {
				...
				for _, r := range req.Requests {
					if err := srv.qbg.Insert(r.Vector.Id, r.Vector.Vector); err != nil {
						t.Fatal(err)
					}
				}
			},
...
func Test_server_SaveIndex(t *testing.T) {
	...
	type test struct {
		...
		fields     fields
		want       want
		checkFunc  func(test, context.Context, Server, service.QBG, want, *payload.Empty, error) error
		beforeFunc func(*testing.T, context.Context, Server, service.QBG)
		afterFunc  func(test)
	}
	defaultCheckFunc := func(test test, ctx context.Context, s Server, n service.QBG, w want, gotRes *payload.Empty, err error) error {
	...
	// it creates a new qbg and server instance with the backup file, and checks if we can retrieve all of wantVecs indexes
	...
	checkBackupFolder := func(fields fields, ctx context.Context, wantVecs []*payload.Insert_Request) error {
		...
		eg, _ := errgroup.New(ctx)
		qbg, err := service.New(fields.svcCfg, append(fields.svcOpts,
			service.WithErrGroup(eg),
			service.WithIndexPath(fields.indexPath))...)
		if err != nil {
			return errors.Errorf("failed to init qbg service, error = %v", err)
		}
		srv, err := New(append(fields.srvOpts, WithQBG(qbg), WithErrGroup(eg))...)
		if err != nil {
	...
	/*
		...
		- Boundary Value Testing
			...
			- case 2.1: success to save index with invalid dimension
				- the invalid index will be removed from QBG and the index file
		- Decision Table Testing
	...
	*/
	tests := []test{
		func() test {
			...
			return test{
				...
				beforeFunc: func(t *testing.T, ctx context.Context, s Server, n service.QBG) {
					t.Helper()
				...
				checkFunc: func(test test, ctx context.Context, s Server, n service.QBG, w want, e *payload.Empty, err error) error {
		...
		func() test {
			return test{
				...
				beforeFunc: func(t *testing.T, ctx context.Context, s Server, n service.QBG) {
					...
					// insert invalid vector to qbg directly
				...
				checkFunc: func(test test, ctx context.Context, s Server, n service.QBG, w want, e *payload.Empty, err error) error {
					...
					// the invalid index will be removed inside qbg
					if err := checkBackupFolder(test.fields, ctx, nil); err != nil {
						return err
					}
	...
	for _, tc := range tests {
		...
		t.Run(test.name, func(tt *testing.T) {
			...
			eg, _ := errgroup.New(ctx)
			qbg, err := service.New(test.fields.svcCfg, append(test.fields.svcOpts,
				service.WithErrGroup(eg),
			...
			if test.beforeFunc != nil {
				test.beforeFunc(tt, ctx, s, qbg)
			}

			gotRes, err := s.SaveIndex(ctx, test.args.in)
			if err := checkFunc(test, ctx, s, qbg, test.want, gotRes, err); err != nil {
				tt.Errorf("error = %v", err)
			}
		})
	}
...
func Test_server_CreateAndSaveIndex(t *testing.T) {
	...
	type test struct {
		...
		fields     fields
		want       want
		checkFunc  func(test test, ctx context.Context, s Server, n service.QBG, w want, gotRes *payload.Empty, err error) error
		beforeFunc func(*testing.T, context.Context, Server, service.QBG, test)
	...
	*/
	tests := []test{
		func() test {
			...
			return test{
				...
				beforeFunc: func(t *testing.T, ctx context.Context, s Server, n service.QBG, test test) {
					t.Helper()
					var err error
	...
	for _, tc := range tests {
		...
		t.Run(test.name, func(tt *testing.T) {
			...
			if test.beforeFunc != nil {
				test.beforeFunc(tt, ctx, s, qbg, test)
			}
...
func Test_server_IndexInfo(t *testing.T) {
	...
	/*
		- Equivalence Class Testing
			- case 1.1: return stored count when QBG is empty
			- case 1.2: return stored count with 100 number of indexes
			- case 2.1: return uncommitted count 0 when QBG is empty
			...
			- case 3.1: return when QBG is indexing
			- case 3.2: return when QBG is not indexing
			- case 4.1: return when QBG is saving index
			- case 4.2: return when QBG is not saving index
		- Boundary Value Testing
			- NONE
		- Decision Table Testing
			- NONE
	*/
	tests := []test{
		{
			name: "Equivalence Class Testing case 1.1: return stored count when QBG is empty",
			args: args{
				in1: &payload.Empty{},
			},
			fields: fields{
		...
		func() test {
			...
			ctx, cancel := context.WithCancel(context.Background())
			eg, _ := errgroup.New(ctx)
			qbg, err := service.New(svcCfg, append(svcOpts, service.WithErrGroup(eg))...)
			if err != nil {
				t.Errorf("failed to init qbg service, error = %v", err)
			}
		...
		{
			name: "Equivalence Class Testing case 2.1: return uncommitted count 0 when QBG is empty",
			args: args{
				in1: &payload.Empty{},
			},
			fields: fields{
		...
		{
			name: "Equivalence Class Testing case 3.1: return when QBG is indexing",
			args: args{
				in1: &payload.Empty{},
			},
			fields: fields{
		...
		{
			name: "Equivalence Class Testing case 3.2: return when QBG is not indexing",
			args: args{
				in1: &payload.Empty{},
			},
			fields: fields{
		...
		func() test {
			...
			return test{
				name: "Equivalence Class Testing case 4.1: return when QBG is saving index",
				args: args{
					in1: &payload.Empty{},
				},
				fields: fields{
		...
		{
			name: "Equivalence Class Testing case 4.2: return when QBG is not saving index",
			args: args{
				in1: &payload.Empty{},
			},
			fields: fields{
