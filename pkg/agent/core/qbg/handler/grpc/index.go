...
func (s *server) CreateIndex(ctx context.Context, c *payload.Control_CreateIndexRequest) (res *payload.Empty, err error) {
	...
	err = s.qbg.CreateIndex(ctx, c.GetPoolSize())
	if err != nil {
		var (
			code    codes.Code
			details = []interface{}{
				...
				&errdetails.ResourceInfo{
					ResourceType: qbgResourceType + "/qbg.CreateIndex",
...
func (s *server) CreateAndSaveIndex(ctx context.Context, c *payload.Control_CreateIndexRequest) (res *payload.Empty, err error) {
	...
	res = new(payload.Empty)
	err = s.qbg.CreateAndSaveIndex(ctx, c.GetPoolSize())
	if err != nil {
		var (
			code    codes.Code
			details = []interface{}{
				...
				&errdetails.ResourceInfo{
					ResourceType: qbgResourceType + "/qbg.CreateAndSaveIndex",
					ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, s.name, s.ip),
				},
			}
		)
...
func (s *server) IndexInfo(ctx context.Context, _ *payload.Empty) (res *payload.Info_Index_Count, err error) {
	...
	return &payload.Info_Index_Count{
		Stored:      uint32(s.qbg.Len()),
		Uncommitted: uint32(s.qbg.InsertVQueueBufferLen() + s.qbg.DeleteVQueueBufferLen()),
		Indexing:    s.qbg.IsIndexing(),
		Saving:      s.qbg.IsSaving(),
	}, nil
}
