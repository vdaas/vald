...
func TestWithQBG(t *testing.T) {
	...
	tests := []test{
		{
			name: "set success when qbg is not nil",
			beforeFunc: func(t *testing.T, args *args, w *want) {
				t.Helper()
				n, err := service.New(&config.QBG{
					Dimension:    1024,
					DistanceType: "cos",
					ObjectType:   "uint8",
		...
		{
			name: "set fail when qbg is nil",
			args: args{
				n: nil,
			},
			want: want{
				obj: new(T),
				err: errors.New("invalid option, name: qbg, val: <nil>"),
			},
		},
	}
