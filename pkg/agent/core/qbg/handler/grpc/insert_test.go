					...
					err: func() error {
						...
						return status.WrapWithInvalidArgument("Insert API Incompatible Dimension Size detected",
							...
							&errdetails.ResourceInfo{
								ResourceType: qbgResourceType + "/qbg.Insert",
	...
	*/
	...
	for _, tc := range tests {
		...
		t.Run(test.name, func(tt *testing.T) {
			...
			eg, _ := errgroup.New(ctx)
			qbg, err := service.New(test.fields.qbgCfg,
				append(test.fields.qbgOpts, service.WithErrGroup(eg))...,
			)
			if err != nil {
				tt.Fatal(err)
			}
...
func Test_server_MultiInsert(t *testing.T) {
	...
	genAlreadyExistsJoinedErr := func(uuids []string, req *payload.Insert_MultiRequest, name, ip string) error {
		var joindErr error
		for _, uuid := range uuids {
			errors.Join(joindErr, errors.ErrUUIDAlreadyExists(uuid))
		}
		return status.WrapWithAlreadyExists(fmt.Sprintf("MultiInsert API uuids %v already exists", uuids),
			joindErr,
			&errdetails.RequestInfo{
			...
			&errdetails.ResourceInfo{
				ResourceType: qbgResourceType + "/qbg.MultiInsert",
				ResourceName: fmt.Sprintf("%s: %s(%s)", apiName, name, ip),
			})
	}
	...
	tests := []test{
		...
		func() test {
			...
			return test{
				...
				want: want{
					err: genAlreadyExistsJoinedErr([]string{req.Requests[0].Vector.Id, req.Requests[1].Vector.Id}, req, name, ip),
				},
			}
		}(),
		func() test {
			...
			for _, r := range req.Requests {
				ids = append(ids, r.Vector.Id)
			}
			wantErr := genAlreadyExistsJoinedErr(ids, req, name, ip)
