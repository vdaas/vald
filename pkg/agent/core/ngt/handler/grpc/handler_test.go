...
import (
	...
	"github.com/vdaas/vald/internal/test/data/charset"
	...
	*/
	tests := []test{
		...
		},
		{
			...
			args: args{
				ctx:      ctx,
				indexId:  charset.Utf8ToSjis("こんにちは"),
				searchId: "こんにちは",
			},
			want: want{
				code: codes.NotFound,
		...
		},
		{
			...
			args: args{
				ctx:      ctx,
				indexId:  charset.Utf8ToEucjp("こんにちは"),
				searchId: "こんにちは",
			},
			want: want{
				code: codes.NotFound,
		...
		},
		{
			...
			args: args{
				ctx:      ctx,
				indexId:  "こんにちは",
				searchId: charset.Utf8ToSjis("こんにちは"),
			},
			want: want{
				code: codes.NotFound,
			},
		...
		},
		{
			...
			args: args{
				ctx:      ctx,
				indexId:  "こんにちは",
				searchId: charset.Utf8ToEucjp("こんにちは"),
			},
			want: want{
				code: codes.NotFound,
			},
	...
	*/
	tests := []test{
		...
		func() test {
			...
			ivec := vector.GenIntVec(vector.Gaussian, 1, invalidDim)[0]
			req := &payload.Insert_Request{
				Vector: &payload.Object_Vector{
					Id:     id,
					Vector: ivec,
		...
		func() test {
			...
			ivec := vector.GenF32Vec(vector.Gaussian, 1, invalidDim)[0]
			req := &payload.Insert_Request{
				Vector: &payload.Object_Vector{
					Id:     id,
					Vector: ivec,
		...
		func() test {
			req := &payload.Insert_Request{
				Vector: &payload.Object_Vector{
					Id:     id,
					Vector: vector.GenIntVec(vector.Gaussian, 1, intVecDim)[0],
				},
			}
		...
		func() test {
			req := &payload.Insert_Request{
				Vector: &payload.Object_Vector{
					Id:     id,
					Vector: vector.GenF32Vec(vector.Gaussian, 1, f32VecDim)[0],
				},
			}
		...
		func() test {
			req := &payload.Insert_Request{
				Vector: &payload.Object_Vector{
					Id:     id,
					Vector: vector.GenIntVec(vector.Uniform, 1, intVecDim)[0],
				},
			}
		...
		func() test {
			req := &payload.Insert_Request{
				Vector: &payload.Object_Vector{
					Id:     id,
					Vector: vector.GenF32Vec(vector.Uniform, 1, f32VecDim)[0],
				},
			}
		...
		// Decision Table Testing
		func() test {
			bVec := vector.GenIntVec(vector.Gaussian, 1, intVecDim)[0] // used in beforeFunc
		...
		func() test {
			bVec := vector.GenIntVec(vector.Gaussian, 1, intVecDim)[0] // use in beforeFunc
...
func Test_server_MultiInsert(t *testing.T) {
	...
	genSameVecMultiInsertReq := func(num int, vec []float32, cfg *payload.Insert_Config) *payload.Insert_MultiRequest {
		req := &payload.Insert_MultiRequest{
			Requests: make([]*payload.Insert_Request, num),
		}
		for i := 0; i < num; i++ {
	...
	tests := []test{
		func() test {
			...
			return test{
				...
				args: args{
					ctx:  ctx,
					reqs: genMultiInsertReq(Uint8, vector.Gaussian, insertNum, intVecDim, nil),
				},
				fields: fields{
					name:              name,
					ip:                ip,
				...
				want: want{
					wantRes: genObjectLocations(insertNum, name, ip),
				},
			}
		}(),
		func() test {
			...
			return test{
				...
				args: args{
					ctx:  ctx,
					reqs: genMultiInsertReq(Float, vector.Gaussian, insertNum, f32VecDim, nil),
				},
				fields: fields{
					name:              name,
					ip:                ip,
		...
		func() test {
			insertNum := 1
			req := genMultiInsertReq(Uint8, vector.Gaussian, insertNum, intVecDim+1, nil)
		...
		func() test {
			insertNum := 1
			req := genMultiInsertReq(Float, vector.Gaussian, insertNum, f32VecDim+1, nil)
		...
		func() test {
			insertNum := 100
			req := genMultiInsertReq(Uint8, vector.Gaussian, insertNum, intVecDim, nil)
			req.Requests[0].Vector.Vector = vector.GenIntVec(vector.Gaussian, 1, intVecDim+1)[0]
		...
		func() test {
			...
			req.Requests[0].Vector.Vector = vector.GenF32Vec(vector.Gaussian, 1, f32VecDim+1)[0]
		...
		func() test {
			...
			for i := 0; i < len(req.Requests)/2; i++ {
				req.Requests[i].Vector.Vector = vector.GenIntVec(vector.Gaussian, 1, intVecDim+1)[0]
			}
		...
		func() test {
			...
			req := genMultiInsertReq(Float, vector.Gaussian, insertNum, f32VecDim, nil)
			for i := 0; i < len(req.Requests)/2; i++ {
				req.Requests[i].Vector.Vector = vector.GenF32Vec(vector.Gaussian, 1, f32VecDim+1)[0]
			}
		...
		func() test {
			...
			return test{
				...
				args: args{
					ctx:  ctx,
					reqs: genSameVecMultiInsertReq(insertNum, vector.GenSameValueVec(intVecDim, 0), nil),
				},
				fields: fields{
					name:              name,
					ip:                ip,
		...
		func() test {
			...
			return test{
				...
				args: args{
					ctx:  ctx,
					reqs: genSameVecMultiInsertReq(insertNum, vector.GenSameValueVec(f32VecDim, 0), nil),
				},
				fields: fields{
					name:              name,
					ip:                ip,
		...
		func() test {
			...
			return test{
				...
				args: args{
					ctx:  ctx,
					reqs: genSameVecMultiInsertReq(insertNum, vector.GenSameValueVec(intVecDim, math.MinInt), nil),
				},
				fields: fields{
					name:              name,
					ip:                ip,
		...
		func() test {
			...
			return test{
				...
				args: args{
					ctx:  ctx,
					reqs: genSameVecMultiInsertReq(insertNum, vector.GenSameValueVec(f32VecDim, -math.MaxFloat32), nil),
				},
				fields: fields{
					name:              name,
					ip:                ip,
		...
		func() test {
			...
			return test{
				...
				args: args{
					ctx:  ctx,
					reqs: genSameVecMultiInsertReq(insertNum, vector.GenSameValueVec(intVecDim, math.MaxUint8), nil),
				},
				fields: fields{
					name:              name,
					ip:                ip,
		...
		func() test {
			insertNum := 100
			req := genMultiInsertReq(Uint8, vector.Gaussian, insertNum, f32VecDim, nil)
			for i := 0; i < len(req.Requests)/2; i++ {
				req.Requests[i].Vector.Id = ""
			}
		...
		func() test {
			insertNum := 100
			req := genSameVecMultiInsertReq(insertNum, make([]float32, maxVecDim), nil)
		...
		func() test {
			...
			return test{
				...
				args: args{
					ctx:  ctx,
					reqs: genSameVecMultiInsertReq(insertNum, vector.GenSameValueVec(f32VecDim, float32(math.NaN())), nil),
				},
				fields: fields{
					name:              name,
					ip:                ip,
		...
		func() test {
			...
			return test{
				...
				args: args{
					ctx:  ctx,
					reqs: genSameVecMultiInsertReq(insertNum, vector.GenSameValueVec(f32VecDim, float32(math.Inf(+1.0))), nil),
				},
				fields: fields{
					name:              name,
					ip:                ip,
		...
		func() test {
			...
			return test{
				...
				args: args{
					ctx:  ctx,
					reqs: genSameVecMultiInsertReq(insertNum, vector.GenSameValueVec(f32VecDim, float32(math.Copysign(0, -1.0))), nil),
				},
				fields: fields{
					name:              name,
					ip:                ip,
		...
		func() test {
			...
			return test{
				...
				beforeFunc: func(t *testing.T, s *server) {
					vecs := vector.GenF32Vec(vector.Gaussian, 2, f32VecDim)
					for i := 0; i < 2; i++ {
	...
	*/
	tests := []test{
		...
		},
		{
			...
			want: want{
				wantUuid: charset.Utf8ToSjis("こんにちは"),
			},
		},
		{
		...
		},
		{
			...
			want: want{
				wantUuid: charset.Utf8ToEucjp("こんにちは"),
			},
		},
		{
	...
	tests := []test{
		...
		},
		{
			...
			args: args{
				ctx:      ctx,
				indexId:  "こんにちは",
				removeId: charset.Utf8ToSjis("こんにちは"),
			},
			want: want{
				code: codes.NotFound,
			},
		...
		},
		{
			...
			args: args{
				ctx:      ctx,
				indexId:  "こんにちは",
				removeId: charset.Utf8ToEucjp("こんにちは"),
			},
			want: want{
				code: codes.NotFound,
			},
