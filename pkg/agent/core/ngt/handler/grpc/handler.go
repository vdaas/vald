...
func (s *server) Exists(
	...
	if !ok {
		...
		err = status.WrapWithNotFound(
			fmt.Sprintf("Exists API meta %s's uuid not found", uid.GetId()),
			err,
...
func (s *server) Search(
	...
	if len(req.GetVector()) != s.ngt.GetDimensionSize() {
		err = errors.ErrIncompatibleDimensionSize(
			len(req.GetVector()),
			int(s.ngt.GetDimensionSize()),
		)
	...
	if err != nil {
		...
		if errors.Is(err, errors.ErrCreateIndexingIsInProgress) {
			err = status.WrapWithResourceExhausted(
				"Search API failed to process search request due to creating index is in progress",
				err,
				&errdetails.RequestInfo{
					RequestId:   req.GetConfig().GetRequestId(),
					ServingData: errdetails.Serialize(req),
...
func (s *server) SearchByID(
	...
	if err != nil {
		...
		if errors.Is(err, errors.ErrCreateIndexingIsInProgress) {
			err = status.WrapWithResourceExhausted(
				"SearchByID API failed to process search request due to create indexing is in progress",
				err,
				&errdetails.RequestInfo{
					RequestId:   req.GetConfig().GetRequestId(),
					ServingData: errdetails.Serialize(req),
...
func toSearchResponse(
	...
	err error,
) (res *payload.Search_Response, rerr error) {
...
func (s *server) StreamSearch(stream vald.Search_StreamSearchServer) (err error) {
	...
	err = grpc.BidirectionalStream(ctx, stream, s.streamConcurrency,
		...
		func(ctx context.Context, data interface{}) (interface{}, error) {
			...
			ctx, sspan := trace.StartSpan(
				ctx,
				apiName+".StreamSearch/requestID-"+req.GetConfig().GetRequestId(),
			)
			defer func() {
				if sspan != nil {
					sspan.End()
...
func (s *server) MultiSearch(
	...
	for i, req := range reqs.Requests {
		...
		s.eg.Go(func() (err error) {
			...
			ctx, sspan := trace.StartSpan(
				ctx,
				fmt.Sprintf("%s.MultiSearch/errgroup.Go/id-%d", apiName, idx),
			)
			defer func() {
				if sspan != nil {
					sspan.End()
...
func (s *server) MultiSearchByID(
	...
	for i, req := range reqs.Requests {
		...
		s.eg.Go(func() error {
			ctx, sspan := trace.StartSpan(
				ctx,
				fmt.Sprintf("%s.MultiSearchByID/errgroup.Go/id-%d", apiName, idx),
			)
			defer func() {
				if sspan != nil {
					sspan.End()
	...
	if err != nil {
		...
		if errors.Is(err, errors.ErrUUIDAlreadyExists(vec.GetId())) {
			err = status.WrapWithAlreadyExists(
				fmt.Sprintf("Insert API uuid %s already exists", vec.GetId()),
				err,
				&errdetails.RequestInfo{
...
func (s *server) StreamInsert(stream vald.Insert_StreamInsertServer) (err error) {
	...
	if err != nil {
		st, msg, err := status.ParseError(
			err,
			codes.Internal,
			"failed to parse StreamInsert gRPC error response",
		)
		if span != nil {
			span.SetStatus(trace.FromGRPCStatus(st.Code(), msg))
		}
...
func (s *server) MultiInsert(
	...
	for _, req := range reqs.GetRequests() {
		...
		if len(vec.GetVector()) != s.ngt.GetDimensionSize() {
			...
			err = status.WrapWithInvalidArgument(
				"MultiInsert API Incompatible Dimension Size detected",
				err,
				&errdetails.RequestInfo{
					RequestId:   vec.GetId(),
					ServingData: errdetails.Serialize(req),
	...
	if err != nil {
		...
		if alreadyExistsIDs := func() []string {
			...
			err = status.WrapWithAlreadyExists(
				fmt.Sprintf("MultiInsert API uuids %v already exists", alreadyExistsIDs),
				err,
				&errdetails.RequestInfo{
...
func (s *server) Update(
	...
	if err != nil {
		...
		if errors.Is(err, errors.ErrObjectIDNotFound(vec.GetId())) {
			err = status.WrapWithNotFound(
				fmt.Sprintf("Update API uuid %s not found", vec.GetId()),
				err,
				&errdetails.RequestInfo{
					RequestId:   req.GetVector().GetId(),
					ServingData: errdetails.Serialize(req),
...
func (s *server) StreamUpdate(stream vald.Update_StreamUpdateServer) (err error) {
	...
	if err != nil {
		st, msg, err := status.ParseError(
			err,
			codes.Internal,
			"failed to parse StreamUpdate gRPC error response",
		)
		if span != nil {
			span.SetStatus(trace.FromGRPCStatus(st.Code(), msg))
		}
...
func (s *server) MultiUpdate(
	...
	for _, req := range reqs.GetRequests() {
		...
		if len(vec.GetVector()) != s.ngt.GetDimensionSize() {
			...
			err = status.WrapWithInvalidArgument(
				"MultiUpdate API Incompatible Dimension Size detected",
				err,
				&errdetails.RequestInfo{
					RequestId:   vec.GetId(),
					ServingData: errdetails.Serialize(req),
	...
	if err != nil {
		...
		if notFoundIDs := func() []string {
			...
			err = status.WrapWithNotFound(
				fmt.Sprintf("MultiUpdate API uuids %v not found", notFoundIDs),
				err,
				&errdetails.RequestInfo{
					RequestId:   strings.Join(uuids, ", "),
					ServingData: errdetails.Serialize(reqs),
...
func (s *server) Upsert(
	...
	if err != nil {
		st, msg, err := status.ParseError(
			err,
			codes.Internal,
			"failed to parse Upsert gRPC error response",
			&errdetails.RequestInfo{
				RequestId:   req.GetVector().GetId(),
				ServingData: errdetails.Serialize(req),
			},
...
func (s *server) StreamUpsert(stream vald.Upsert_StreamUpsertServer) (err error) {
	...
	if err != nil {
		st, msg, err := status.ParseError(
			err,
			codes.Internal,
			"failed to parse StreamUpsert gRPC error response",
		)
		if span != nil {
			span.SetStatus(trace.FromGRPCStatus(st.Code(), msg))
		}
...
func (s *server) MultiUpsert(
	...
	for _, req := range reqs.GetRequests() {
		...
		if len(vec.GetVector()) != s.ngt.GetDimensionSize() {
			...
			err = status.WrapWithInvalidArgument(
				"MultiUpsert API Incompatible Dimension Size detected",
				err,
				&errdetails.RequestInfo{
					RequestId:   vec.GetId(),
					ServingData: errdetails.Serialize(req),
...
func (s *server) StreamRemove(stream vald.Remove_StreamRemoveServer) (err error) {
	...
	if err != nil {
		st, msg, err := status.ParseError(
			err,
			codes.Internal,
			"failed to parse StreamRemove gRPC error response",
		)
		if span != nil {
			span.SetStatus(trace.FromGRPCStatus(st.Code(), msg))
		}
...
func (s *server) MultiRemove(
	...
	if err != nil {
		...
		if notFoundIDs := func() []string {
			...
			err = status.WrapWithNotFound(
				fmt.Sprintf("MultiRemove API uuids %v not found", notFoundIDs),
				err,
				&errdetails.RequestInfo{
					RequestId:   strings.Join(uuids, ", "),
					ServingData: errdetails.Serialize(reqs),
...
func (s *server) StreamGetObject(stream vald.Object_StreamGetObjectServer) (err error) {
	...
	if err != nil {
		st, msg, err := status.ParseError(
			err,
			codes.Internal,
			"failed to parse StreamGetObject gRPC error response",
		)
		if span != nil {
			span.SetStatus(trace.FromGRPCStatus(st.Code(), msg))
		}
	...
	if err != nil {
		if err == errors.ErrUncommittedIndexNotFound {
			err = status.WrapWithFailedPrecondition(
				fmt.Sprintf(
					"CreateIndex API failed to create indexes pool_size = %d",
					c.GetPoolSize(),
				),
				err,
				&errdetails.RequestInfo{
...
func (s *server) CreateAndSaveIndex(
	...
	if err != nil {
		if err == errors.ErrUncommittedIndexNotFound {
			err = status.WrapWithFailedPrecondition(
				fmt.Sprintf(
					"CreateAndSaveIndex API failed to create indexes pool_size = %d",
					c.GetPoolSize(),
				),
				err,
				&errdetails.RequestInfo{
