...
func (s *server) Remove(
	...
	id := req.GetId()
	uuid := id.GetId()
	if err = s.validateUUID(span, vald.RemoveRPCName, faissResourceType+"/faiss.Remove",
		uuid, req); err != nil {
		return nil, err
	}
	...
	if err != nil {
		...
		if errors.Is(err, errors.ErrObjectIDNotFound(uuid)) {
			err = status.WrapWithNotFound(fmt.Sprintf("Remove API uuid %s not found", uuid), err,
				...
				s.resourceInfo(faissResourceType+"/faiss.Remove"))
			log.Warn(err)
			...
			err = status.WrapWithInternal("Remove API failed", err,
				...
				s.resourceInfo(faissResourceType+"/faiss.Remove"), info.Get())
			log.Error(err)
