...
func (b *bs) Reader(ctx context.Context) (r io.ReadCloser, err error) {
	...
	if b.compressor != nil {
		cr, cerr := b.compressor.Reader(r)
		if cerr != nil {
			// Close the bucket reader we already opened; leaving it open when
			// wrapping with the decompressor fails leaks the stream, and this
			// repeats on every restorer backoff retry.
			return nil, errors.Join(cerr, r.Close())
		}
		r = cr
	}
...
func (b *bs) Writer(ctx context.Context) (w io.WriteCloser, err error) {
	...
	if b.compressor != nil {
		cw, cerr := b.compressor.Writer(w)
		if cerr != nil {
			// Close the bucket writer we already opened; leaving it open when
			// wrapping with the compressor fails leaks the stream, and this
			// repeats on every backoff retry.
			return nil, errors.Join(cerr, w.Close())
		}
		w = cw
	}
