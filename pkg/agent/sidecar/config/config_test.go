...
func Test_sidecarMode_String(t *testing.T) {
	type want struct {
		want string
	}
	type test struct {
...
func TestSidecarMode(t *testing.T) {
	...
	type test struct {
		...
		args       args
		want       want
		checkFunc  func(want, sidecarMode) error
		beforeFunc func(args)
		afterFunc  func(args)
	}
	defaultCheckFunc := func(w want, got sidecarMode) error {
		if !reflect.DeepEqual(got, w.want) {
			return errors.Errorf("got = %v, want %v", got, w.want)
		}
		return nil
