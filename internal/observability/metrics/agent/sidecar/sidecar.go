...
func (sm *sidecarMetrics) Register(m metrics.Meter) error {
	...
	_, err = m.RegisterCallback(
		func(_ context.Context, o api.Observer) error {
			...
			opts := attribute.ToMeasurementOption(
			...
			o.ObserveInt64(uploadTotal, 1, opts)
			o.ObserveInt64(uploadBytes, sm.info.Bytes, opts)
			latencyMillis := float64(sm.info.EndTime.Sub(sm.info.StartTime)) / float64(time.Millisecond)
			o.ObserveFloat64(uploadLatency, latencyMillis, opts)
			sm.info = nil
