...
const (
	indexCountMetricsName        = "agent_core_qbg_index_count"
	indexCountMetricsDescription = "Agent QBG index count"

	uncommittedIndexCountMetricsName        = "agent_core_qbg_uncommitted_index_count"
	uncommittedIndexCountMetricsDescription = "Agent QBG index count"

	insertVQueueCountMetricsName        = "agent_core_qbg_insert_vqueue_count"
	insertVQueueCountMetricsDescription = "Agent QBG insert vqueue count"

	deleteVQueueCountMetricsName        = "agent_core_qbg_delete_vqueue_count"
	deleteVQueueCountMetricsDescription = "Agent QBG delete vqueue count"

	completedCreateIndexTotalMetricsName        = "agent_core_qbg_completed_create_index_total"
	completedCreateIndexTotalMetricsDescription = "The cumulative count of completed create index execution"

	executedProactiveGCTotalMetricsName        = "agent_core_qbg_executed_proactive_gc_total"
	executedProactiveGCTotalMetricsDescription = "The cumulative count of proactive GC execution"

	isIndexingMetricsName        = "agent_core_qbg_is_indexing"
	isIndexingMetricsDescription = "Currently indexing or no"

	isSavingMetricsName        = "agent_core_qbg_is_saving"
	isSavingMetricsDescription = "Currently saving or not"

	brokenIndexStoreCountMetricsName        = "agent_core_qbg_broken_index_store_count"
	brokenIndexStoreCountMetricsDescription = "How many broken index generations have been stored"
)

type qbgMetrics struct {
	qbg service.QBG
}

func New(n service.QBG) metrics.Metric {
	return &qbgMetrics{
		qbg: n,
	}
}

func (n *qbgMetrics) View() ([]metrics.View, error) {
	return []metrics.View{
		view.NewView(
			view.Instrument{
...
func (n *qbgMetrics) Register(m metrics.Meter) error {
	...
	_, err = m.RegisterCallback(
		func(_ context.Context, o api.Observer) error {
			...
			o.ObserveInt64(indexCount, int64(n.qbg.Len()))
			o.ObserveInt64(uncommittedIndexCount, int64(n.qbg.InsertVQueueBufferLen()+n.qbg.DeleteVQueueBufferLen()))
			o.ObserveInt64(insertVQueueCount, int64(n.qbg.InsertVQueueBufferLen()))
			o.ObserveInt64(deleteVQueueCount, int64(int64(n.qbg.DeleteVQueueBufferLen())))
			o.ObserveInt64(completedCreateIndexTotal, int64(n.qbg.NumberOfCreateIndexExecution()))
			o.ObserveInt64(executedProactiveGCTotal, int64(n.qbg.NumberOfProactiveGCExecution()))
