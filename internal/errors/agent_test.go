...
func TestErrInvalidDimensionSize(t *testing.T) {
	...
	tests := []test{
		...
		{
			name: "return an ErrInvalidDimensionSize error when current and limit are negative values",
			args: args{
				current: -100,
				limit:   -100,
			},
			want: want{
				want: Errorf("dimension size %d is invalid, the supporting dimension size must be between 2 ~ %d", -100, -100),
			},
		},
		{
	...
	tests := []test{
		{
			...
			want: want{
				want: New("uuid 550e8400-e29b-41d4 index already exists"),
			},
		},
		{
			...
			want: want{
				want: New("uuid  index already exists"),
			},
		},
	}
	...
	tests := []test{
		{
			...
			want: want{
				want: New("object uuid 1234's metadata not found"),
			},
		},
		{
			...
			want: want{
				want: Errorf("object uuid %d's metadata not found", math.MaxUint32),
			},
		},
		{
			...
			want: want{
				want: New("object uuid not found"),
			},
		},
	}
	...
	tests := []test{
		{
			...
			want: want{
				want: New("uuid 550e8400-e29b-41d4's object id not found"),
			},
		},
		{
			...
			want: want{
				want: New("uuid 's object id not found"),
			},
		},
	}
