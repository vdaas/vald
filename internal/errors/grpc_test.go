...
func TestErrInvalidGRPCPort(t *testing.T) {
	...
	tests := []test{
		...
		},
		{
			...
			want: want{
				want: New(
					"invalid gRPC client connection port to addr: 127.0.0.1,\thost: \t port: 8080",
				),
			},
		},
		{
	...
	tests := []test{
		{
			...
			want: want{
				want: New(
					"vald internal gRPC client could not find ip addrs for gateway.vald.svc.cluster.local",
				),
			},
		},
		{
