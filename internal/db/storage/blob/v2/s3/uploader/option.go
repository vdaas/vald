...
var defaultOptions = []Option{
	...
	WithPartSize(defaultUploadPartSize),
	WithMaxUploadParts(maxUploadParts),
	WithConcurrency(defaultUploadConcurrency),
...
func WithAPIClient(mdc manager.UploadAPIClient) Option {
	return func(c *client) error {
		if mdc != nil {
			c.client = mdc
		}
...
func WithMaxUploadParts(num int32) Option {
	return func(c *client) error {
		if num > 0 {
			c.maxUploadParts = num
		}
		return nil
	}
}
...
func WithBufferProvider(bp manager.ReadSeekerWriteToProvider) Option {
	return func(c *client) error {
		if bp != nil {
			c.bufferProvider = bp
		}
