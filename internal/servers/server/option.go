...
// If size is greater than zero or equal to -1, it applies the corresponding grpc.MaxRecvMsgSize setting and updates the server's recorded max message size when the provided size is larger; otherwise the option has no effect.
