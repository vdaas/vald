...
func (p *pool) refreshConn(
	ctx context.Context, eg errgroup.Group, idx uint64, pc *poolConn, addr string,
) {
...
func (p *pool) connect(ctx context.Context, ips ...string) (c Conn, err error) {
	...
	err = p.loop(egctx, func(ictx context.Context, idx uint64, pc *poolConn) bool {
		p.refreshConn(ictx, eg, idx, pc, net.JoinHostPort(ips[idx%uint64(len(ips))], p.port))
		return true
	})
	if errors.IsNot(err, context.DeadlineExceeded, context.Canceled) {
		return p, err
...
func (p *pool) singleTargetConnect(ctx context.Context, addr string) (Conn, error) {
	...
	eg, egctx := errgroup.New(ctx)
	err := p.loop(egctx, func(ictx context.Context, idx uint64, pc *poolConn) bool {
		p.refreshConn(ictx, eg, idx, pc, addr)
		return true
	})
	if errors.IsNot(err, context.DeadlineExceeded, context.Canceled) {
		return p, err
...
func (p *pool) getHealthyConn(ctx context.Context) (pc *poolConn, ok bool) {
	...
	for _, idx := range rc {
		// we should reload the connection from the pool, because the connection might be updated by refreshConn
		_, pc = p.load(idx)
		if pc != nil && pc.conn != nil && pc.conn.GetState() != connectivity.Shutdown {
			return pc, true
		}
...
func (p *pool) IsHealthy(ctx context.Context) bool {
	...
	for idx := range sz {
		select {
		case <-ctx.Done():
			log.Debugf("health check loop for addr=%s returned error: %v", p.addr, ctx.Err())
			if p.isIPAddr {
		...
		if pc != nil && pc.conn != nil {
			...
			if healthy {
				...
				log.Debugf("unhealthy connection detected for %s pool %d/%d len %d is unhealthy (state: %s)",
					pc.addr, idx+1, p.Size(), sz, state.String())
			}
		} else {
			// Do not block health check with dial
			p.refreshConn(ctx, p.errGroup, idx, pc, p.addr)
			// Assume unhealthy if nil for now
			log.Debugf("nil pool connection detected for %s pool %d/%d len %d is unhealthy", p.addr, idx+1, p.Size(), sz)
		}
	}
	...
	if healthyCount == 0 {
		log.Warnf("no connection pool member is healthy for addr=%s size=%d, len=%d", p.addr, p.Size(), sz)
		return false
	}
	if p.isIPAddr {
		return healthyCount == sz
