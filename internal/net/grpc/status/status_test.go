...
import (
	...
	grpcerr "github.com/vdaas/vald/apis/grpc/errors"
...
func Test_newStatus(t *testing.T) {
	...
	defaultCheckFunc := func(w want, gotSt *status.Status) error {
		if !reflect.DeepEqual(gotSt, w.wantSt) {
			return errors.Errorf("got = %v, want %v", gotSt, w.wantSt)
		}
		return nil
	}
	tests := []test{
...
func TestFromError(t *testing.T) {
	...
	type want struct {
		want *grpcerr.Errors_RPC
	}
	type test struct {
		name       string
		args       args
		want       want
		checkFunc  func(want, *grpcerr.Errors_RPC) error
		beforeFunc func(args)
		afterFunc  func(args)
	}
	defaultCheckFunc := func(w want, got *grpcerr.Errors_RPC) error {
