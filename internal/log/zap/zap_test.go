...
func Test_logger_log(t *testing.T) {
	...
	tests := []test{
		func() test {
			...
			return test{
				name: "call sugarFunc when the length of vals is 1",
				args: args{
					loggerFunc: nil,
					sugarFunc: func(args ...interface{}) {
						count++
				...
				checkFunc: func(w want) error {
					if count != 1 {
						return errors.New("sugarFunc is not called")
					}
					return nil
				},
			}
		...
		func() test {
			...
			return test{
				name: "call sugarFunc when the length of vals is greater than 1 and the first argument is not string",
				args: args{
					loggerFunc: nil,
					sugarFunc: func(args ...interface{}) {
						count++
		...
		func() test {
			...
			return test{
				name: "call loggerFunc when the length of vals is 2 and the first argument is string",
				args: args{
					loggerFunc: func(msg string, fields ...zapcore.Field) {
						count++
					},
				...
				checkFunc: func(w want) error {
					if count != 1 {
						return errors.New("loggerFunc is not called")
					}
					return nil
				},
			}
		...
		func() test {
			...
			return test{
				name: "call loggerFunc when the length of vals is greater than 2 and the first argument is string",
				args: args{
					loggerFunc: func(msg string, fields ...zapcore.Field) {
						count++
					},
	...
	for _, test := range tests {
		t.Run(test.name, func(tt *testing.T) {
			...
			l.log(test.args.loggerFunc, test.args.sugarFunc, test.args.vals...)
