// Package valdrelease provides primitives to interact with the openapi HTTP API.
//
...
// AgentUnhealthyPodEvictionPolicy controls whether unhealthy pods can be evicted based on the application's healthy pod count, supporting either cautious or permissive eviction.
...
// AgentNgtDistanceType distance type. it should be `l1`, `l2`, `angle`, `hamming`, `cosine`,`poincare`, `lorentz`, `jaccard`, `sparsejaccard`, `normalizedangle` or `normalizedcosine` or `innerproduct`. for further details about NGT libraries supported distance is https://github.com/NGT-labs/NGT/wiki/Command-Quick-Reference and vald agent's supported NGT distance type is https://pkg.go.dev/github.com/vdaas/vald/internal/core/algorithm/ngt#pkg-constants
type AgentNgtDistanceType string

// AgentNgtObjectType object type. it should be `float` or `uint8` or `float16`. for further details: https://github.com/NGT-labs/NGT/wiki/Command-Quick-Reference
...
// DiscovererUnhealthyPodEvictionPolicy controls whether unhealthy pods can be evicted based on the application's healthy pod count, supporting either cautious or permissive eviction.
...
// GatewayFilterUnhealthyPodEvictionPolicy controls whether unhealthy pods can be evicted based on the application's healthy pod count, supporting either cautious or permissive eviction.
...
// GatewayLbUnhealthyPodEvictionPolicy controls whether unhealthy pods can be evicted based on the application's healthy pod count, supporting either cautious or permissive eviction.
...
// GatewayMirrorUnhealthyPodEvictionPolicy controls whether unhealthy pods can be evicted based on the application's healthy pod count, supporting either cautious or permissive eviction.
...
// GatewayMirrorAffinityNodeAffinityRequiredDuringSchedulingIgnoredDuringExecution defines model for gateway_mirror_affinity_nodeAffinity_requiredDuringSchedulingIgnoredDuringExecution.
...
// ManagerIndexUnhealthyPodEvictionPolicy controls whether unhealthy pods can be evicted based on the application's healthy pod count, supporting either cautious or permissive eviction.
