...
func notFoundErr() error {
	return apierrors.NewNotFound(schema.GroupResource{Resource: "pods"}, "missing")
}
...
func TestListReconciler_Reconcile(t *testing.T) {
	...
	tests := []test{
		...
		},
		{
			...
			want: want{
				result:       reconcile.Result{Requeue: true, RequeueAfter: defaultErrorRequeueDuration},
				onErrorFired: true,
			},
		},
		{
			...
			client: func(t *testing.T) k8s.Client {
				t.Helper()
				return &errClient{err: notFoundErr()}
			},
			opts: func(_, errored *int) []ListOption[*corev1.PodList] {
				return []ListOption[*corev1.PodList]{
					WithOnError[*corev1.PodList](func(error) { *errored++ }),
			...
			want: want{
				result:       reconcile.Result{Requeue: true, RequeueAfter: defaultNotFoundRequeueDuration},
				onErrorFired: true,
			},
		},
	}
