...
func (c *Client[T, L]) DeleteAndWait(ctx context.Context, obj T, opts ...ListOption) error {
	...
	eg.Go(func() error {
		...
		for {
			...
			case event, ok := <-watcher.ResultChan():
				// A closed channel (watcher.Stop, or the apiserver dropping
				// the watch) yields ok==false forever; without this guard the
				// receive would return a zero Event on every iteration and the
				// goroutine would busy-spin instead of terminating.
				if !ok {
					return errors.Errorf(
						"watch for %T(%s) closed before a Deleted event was observed", obj, obj.GetName())
				}
				if event.Type == watch.Deleted {
					// Only a Deleted event for THIS object confirms the wait. The
					// watch is scoped by the (namespace+label) selector, which can
					// match sibling objects — e.g. a freshly created replacement
					// sharing the same replica label — and returning on any of their
					// deletions would falsely report obj as gone. c.Delete below is
					// issued after the watch is established, so obj's own Deleted
					// event is guaranteed to arrive; this cannot hang.
					if acc, ok := event.Object.(interface{ GetName() string }); ok && acc.GetName() != obj.GetName() {
						log.Debugf("observed Deleted event for a different %T(%s) matching the selector; still waiting for (%s)", obj, acc.GetName(), obj.GetName())
						continue
					}
	...
	if err := c.Delete(ctx, obj); err != nil {
		// Stop the watch so the goroutine above unblocks (ResultChan closes,
		// the ok guard fires) and reap it, instead of returning early and
		// leaking a goroutine that spins on the closed channel. Its error is
		// the expected "watch closed" signal, so log it rather than let it
		// mask the real delete failure we return.
		watcher.Stop()
		if werr := eg.Wait(); werr != nil {
			log.Debugf("watch for %T(%s) stopped after delete failure: %v", obj, obj.GetName(), werr)
		}
