...
// scheme is used for SetControllerReference; the client for the
...
func (s *Syncer) Sync(
	...
	for _, obj := range desired {
		...
		if err := SetControllerReference(owner, obj, s.scheme); err != nil {
			return opes, errors.Wrap(err, "failed to set controller reference")
		}

		// CreateOrUpdateObject overwrites obj with the live cluster state
		...
		ope, err := CreateOrUpdateObject(ctx, s.api, obj, func() error {
...
// CreateOrUpdateObject's Get onto the freshly fetched obj: every non-metadata
...
func (s *Syncer) prune(
	...
	for _, obj := range items {
		...
		if _, keep := applied[key]; !keep {
			if err := DeleteObject(ctx, s.api, obj); err != nil && !apierrors.IsNotFound(err) {
