...
// NewObjectClient returns an ObjectClient for the object type T backed by api.
...
// Refresh re-fetches obj from the cluster using its own name and namespace.
func (c *ObjectClient[T, PT]) Refresh(ctx context.Context, obj PT) (PT, error) {
	return RefreshObject(ctx, c.api, obj)
}

// Create persists obj as a new object.
func (c *ObjectClient[T, PT]) Create(ctx context.Context, obj PT) error {
	return c.api.Create(ctx, obj)
}

// Update persists modifications to obj.
func (c *ObjectClient[T, PT]) Update(ctx context.Context, obj PT) error {
	return c.api.Update(ctx, obj)
}

// UpdateStatus persists the status subresource of obj.
func (c *ObjectClient[T, PT]) UpdateStatus(ctx context.Context, obj PT) error {
	return UpdateObjectStatus(ctx, c.api, obj)
}

// Delete removes obj from the cluster.
func (c *ObjectClient[T, PT]) Delete(ctx context.Context, obj PT) error {
	return DeleteObject(ctx, c.api, obj)
}

// CreateOrUpdate creates obj when missing, otherwise applies mutate and
// updates it, returning the performed operation.
func (c *ObjectClient[T, PT]) CreateOrUpdate(
	ctx context.Context, obj PT, mutate func() error,
) (OperationResult, error) {
	return CreateOrUpdateObject(ctx, c.api, obj, mutate)
}
...
func (c *ObjectClient[T, PT]) Wait(
	...
	for {
		...
		case <-timeout.C:
			return false, errors.Errorf("timeout waiting for %s/%s", namespace, name)
		case <-ticker.C:
