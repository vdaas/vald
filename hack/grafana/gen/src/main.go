...
func ValdClusterOverview() *dashboard.DashboardBuilder {
	...
	addLatencyPanel(builder, "Vald LB Gateway: ", "$ReplicaSet", "$ValdGatewayPodName", `=~".*"`)
	addCompletedRPCPanel(builder, "Vald Agent: ", "$ReplicaSet", "$ValdAgentPodName")
	addLatencyPanel(builder, "Vald Agent: ", "$ReplicaSet", "$ValdAgentPodName", `!~".*Index$"`)
	addBackoffPanel(builder)
	addBackoffPerRPCPanel(builder)
	addCircuitBreakerState(builder)
	addLatencyPanel(builder, "Vald Agent Index: ", "$ReplicaSet", "$ValdAgentPodName", `=~".*Index$"`)
...
func ValdLBGateway() *dashboard.DashboardBuilder {
	...
	addLatencyPanel(dashboard, "", "$ReplicaSet", "$PodName", `=~".*"`)
