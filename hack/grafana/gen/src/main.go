...
func ValdClusterOverview() *dashboard.DashboardBuilder {
	...
	addLatencyPanel(builder, "Vald LB Gateway: ", "$ReplicaSet", "$ValdGatewayPodName", "$ValdGatewayContainerName", `=~".*"`)
	addCompletedRPCPanel(builder, "Vald Agent: ", "$ReplicaSet", "$ValdAgentPodName")
	addLatencyPanel(builder, "Vald Agent: ", "$ReplicaSet", "$ValdAgentPodName", "$ValdAgentContainerName", `!~".*Index$"`)
	addBackoffPanel(builder)
	addBackoffPerRPCPanel(builder)
	addCircuitBreakerState(builder)
	addLatencyPanel(builder, "Vald Agent Index: ", "$ReplicaSet", "$ValdAgentPodName", "$ValdAgentContainerName", `=~".*Index$"`)
...
func ValdLBGateway() *dashboard.DashboardBuilder {
	...
	addLatencyPanel(dashboard, "", "$ReplicaSet", "$PodName", "$ContainerName", `=~".*"`)
...
func ValdAgent() *dashboard.DashboardBuilder {
	...
	addLatencyPanel(builder, "", "$ReplicaSet", "$PodName", "$ContainerName", `!~".*SaveIndex"`)
	addGCPanel(builder, "$ReplicaSet")
	addGoroutinePanel(builder, "$ReplicaSet", "$PodName")
	return builder.Time("now-3h", "now")
}
