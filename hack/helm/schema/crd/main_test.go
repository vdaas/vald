...
func Test_genSchema(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(test.name, func(tt *testing.T) {
			...
			err := genSchema(test.args.path, test.args.nindent)
...
func Test_newSpec(t *testing.T) {
	...
	type test struct {
		...
		args       args
		want       want
		checkFunc  func(want, *Spec) error
		beforeFunc func(args)
		afterFunc  func(args)
	}
	defaultCheckFunc := func(w want, got *Spec) error {
		if !reflect.DeepEqual(got, w.want) {
			return errors.Errorf("got: \"%#v\",\n\t\t\t\twant: \"%#v\"", got, w.want)
		}
		return nil
...
func Test_indent(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(test.name, func(tt *testing.T) {
			...
			got := indent(test.args.text, test.args.nindent)
			if err := checkFunc(test.want, got); err != nil {
				tt.Errorf("error = %v", err)
			}
		})
