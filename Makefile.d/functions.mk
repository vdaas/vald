...
# Linker flags shared by every go build/test link step. -w strips the DWARF
# debug tables from release binaries (pre-refactor this was a literal -w inside
# go-build); override with GO_LDFLAGS= to keep debug info for local debugging.
GO_LDFLAGS ?= -w
...
# go-tool-install installs every tool listed in hack/go.tools in parallel.
# Referenced by the gopls/oapi-codegen targets in Makefile.d/tools.mk; an
# undefined call would expand to an empty recipe and silently no-op them.
define go-tool-install
	cat $(ROOTDIR)/hack/go.tools | \
	xargs $(XARGS_NO_RUN_IF_EMPTY) -I {} -P $(CORES) \
	sh -c 'if ! out=$$( $(GO_ENV_VARS) go install -ldflags="-w" {} 2>&1 >/dev/null); then echo "--- Failed to install {} ---" >&2; echo "$$out" >&2; exit 255; fi';
endef
...
define fetch-version
	@{ RESULT=$$($(2)) || RESULT=""; if [ -n "$$RESULT" ]; then printf '%s\n' "$$RESULT" > $(1); else echo "$(notdir $(1)): fetch failed or returned empty — keeping existing value" >&2; fi; }
endef
...
define go-build
	$(call go-base,build, \
		...
		-mod=readonly, \
		$(GO_LDFLAGS) $2 -extldflags '$(strip $3)' \
...
define run-v2-e2e-crud-test
	...
	E2E_MANIFEST_PATH="$(E2E_MANIFEST_PATH)" \
