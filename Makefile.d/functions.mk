...
GO_TEST_FLAGS = $(GO_TEST_BASE_FLAGS) -ldflags="$(GO_STATIC_LDFLAGS)"
...
GOTESTS_GEN_SCRIPT = \
	...
	gotests -w -template_dir $(ROOTDIR)/assets/test/templates/common $$GOTESTS_OPTION $${f/%.go/_test.go};
...
GOTESTS_OPTION_GEN_SCRIPT = \
	...
	gotests -w -template_dir $(ROOTDIR)/assets/test/templates/option $$GOTESTS_OPTION $${f/%.go/_test.go};
...
define cmake-install
	...
	if echo "$1" | grep -qE "\.git$$" || [ -n "$5" ]; then \
		git clone --depth 1 $$(if [ -n "$5" ]; then echo "--branch $5"; fi) $1 $(TEMP_DIR)/$2; \
	else \
